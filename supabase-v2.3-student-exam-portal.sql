...
update public.exam_registrations set status = 'assigned', assigned_at = coalesce(assigned_at, registered_at) where status is null or status not in ('requested','assigned','cancelled','refunded');
