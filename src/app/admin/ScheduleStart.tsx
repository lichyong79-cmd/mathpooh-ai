...
export default function ScheduleStart({onSlotChange}:{onSlotChange?:(slot:any)=>void}){
 ...
 async function changeGate(row:any,allow:boolean){
  ...
  try{
   const response=await fetch("/api/admin/exam-slots",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({action:allow?"override-gate":"revoke-gate",cycleId:id,membershipId:row.id})});
 ...
 async function act(action:string){
  ...
  const requested=id;
  if(action==="start"){
   const extra=blocked.length?"\n\n⚠ SOS 미완료 "+blocked.length+"명은 제외됩니다.":"";
