...
export default function AiGeneratedBankPage(){
 ...
 const filtered=useMemo(()=>questions.filter(q=>!q.verification?.errorReview?.open&&(kind==="전체"||q.generation_kind===kind)&&(!keyword||`${q.subject} ${q.major_unit} ${q.subunit} ${q.topic} ${q.core_type}`.toLowerCase().includes(keyword.toLowerCase()))),[questions,kind,keyword]);
 ...
 async function reportError(q:any){
  const reason=window.prompt("오류 사유를 입력하세요. 오류 보관함으로 이동하고 새 배정에서 제외됩니다.");if(!reason?.trim())return;
  setRunning(true);setError("");
  try{const r=await fetch("/api/admin/problem-errors",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({id:q.id,kind:"ai",action:"report",reason})});const d=await r.json();if(!r.ok)throw Error(d.message);setNotice("오류문항 보관함으로 이동했습니다.");await load();}catch(e){setError((e as Error).message);}finally{setRunning(false);}
 }
 ...
 return <AdminPortalShell current="ai-generated-bank"><main className="ai-bank-page">
  <header><div><small>MATHPOOH SOS · AI GENERATED BANK</small><h1>AI 생성 문제은행</h1><p>2차훈련·3제 굳히기에서 생성·검증된 문항을 영구 보관합니다.</p></div><div className="ai-bank-actions"><a href="/admin/problem-errors">오류문항 보관함 →</a><button onClick={()=>void runNext()} disabled={running} className="run-now">{running?"생성 중...":"지금 1건 생성"}</button><button onClick={()=>void reviveStuck()} disabled={running} className="revive-stuck">멈춘 작업 되살리기</button><button onClick={()=>void load()} disabled={running}>새로고침</button></div></header>
