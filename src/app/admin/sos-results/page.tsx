  ...
  return <AdminPortalShell current="sos-learning">
    <main className="progress-page">
      <header className="top">
        <div><small>MATHPOOH SOS · RESULT REVIEW</small><h1>SOS 결과분석</h1><p>완료된 진단의 문항별 정오답·시간·이탈로그·학생 풀이사진을 확인합니다.</p><nav className="sos-tabs"><button onClick={()=>location.href="/admin?menu=sos-learning"}>배정</button><button onClick={()=>location.href="/admin/sos-progress"}>진행</button><button className="active">결과</button></nav></div>
      ...
      <section className="filters">
        ...
        <div className="result-subtabs"><button className={phase==="DIAGNOSIS"?"active":""} onClick={()=>setPhase("DIAGNOSIS")}>진단결과</button><button className={phase==="TRAINING"?"active":""} onClick={()=>setPhase("TRAINING")}>훈련결과</button></div>
      ...
      {selected?<section className="detail-panel">
        ...
        <div className="problem-grid">
          {(selected.items??[]).map((item:any,index:number)=><article className={`problem-card ${item.isCorrect===true?"correct":item.isCorrect===false?"wrong":""}`} key={item.id}>
            ...
            </div>:<div className="review-result"><small>오답 재풀이</small><b>{item.reviewAnswer?`${item.reviewAnswer} · ${item.reviewIsCorrect===true?"정답":item.reviewIsCorrect===false?"오답":"미채점"}`:"대상 아님/미진행"}</b><span>{item.reviewResponseSeconds?duration(item.reviewResponseSeconds):""}</span>{item.generated&&item.problem?.generatedSolution?<details><summary>AI 생성 해설</summary><p>{item.problem.generatedSolution}</p></details>:null}</div>}
          </article>)}
        </div>
      </section>:null}
