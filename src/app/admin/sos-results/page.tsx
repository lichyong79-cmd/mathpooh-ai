...
export default function SosResultsPage(){
  ...
  }).sort((a:any,b:any)=>String(b.learningCycle?.startDate??"").localeCompare(String(a.learningCycle?.startDate??""))||String(a.learningCycle?.name??"회차 미지정").localeCompare(String(b.learningCycle?.name??"회차 미지정"),"ko")||String(a.student?.name??"").localeCompare(String(b.student?.name??""),"ko")||new Date(a.createdAt).getTime()-new Date(b.createdAt).getTime()),[rows,keyword,phase,status,cycle]);
  const cycleKey=(r:any)=>String(r.learningCycle?.id??"UNASSIGNED");
  const cycleTitle=(r:any)=>r.learningCycle?`${r.learningCycle.name} · ${r.learningCycle.dateLabel}`:"회차 미지정";
  ...
  return <AdminPortalShell current="sos-learning">
    <main className="progress-page">
      ...
      <section className="table">
        ...
        </div></Fragment>):<div className="empty">조건에 맞는 진단·훈련이 없습니다.</div>}
      </section>
