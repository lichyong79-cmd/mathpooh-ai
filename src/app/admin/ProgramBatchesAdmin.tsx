...
const scopeLetter = (v: string) => v === "ALGEBRA" ? "A" : v === "ALGEBRA_CALC1" ? "B" : "C";

function FinderCycles({cycles,selected,scopes,count,busy,onToggle,onScope,onSave}:{cycles:any[];selected:string[];scopes:Record<string,string>;count:number;busy:boolean;onToggle:(id:string,count:number)=>void;onScope:(id:string,scope:string)=>void;onSave:()=>void}) {
  return <section className="application-cycle-editor">
    ...
    <div className="date-grid">{cycles.map((cycle: any) => { const id=String(cycle.id); const checked = selected.includes(id); return <label className={checked ? "selected" : ""} key={id}><input type="checkbox" checked={checked} onChange={() => onToggle(id, count)} /><b>{day(cycle.start_date)}</b><small>{cycle.name}</small><select disabled={!checked} value={scopes[id]??"FULL"} onClick={e=>e.stopPropagation()} onChange={e=>onScope(id,e.target.value)}><option value="ALGEBRA">A · 대수</option><option value="ALGEBRA_CALC1">B · 대수+미적Ⅰ</option><option value="FULL">C · 대수+미적Ⅰ+확통</option></select></label>; })}</div>
...
export default function ProgramBatchesAdmin({mode="payments"}:{mode?:"setup"|"payments"}) {
  ...
  const [editingApplicationScopes, setEditingApplicationScopes] = useState<Record<string,string>>({});
  ...
  const beginApplicationEdit = (application: any, batch: any) => {
    ...
    const savedScopes = application.selected_cycle_scopes ?? {};
    setEditingApplicationScopes(Object.fromEntries(applicationCycleIds(application, batch).map((id:string)=>[id,savedScopes[id]??application.scope_code??"FULL"])));
  };
  const toggleApplicationCycle = (id: string, count: number) => {
    ...
    setEditingApplicationScopes(current=>current[id]?current:{...current,[id]:"FULL"});
  };
  const saveApplicationCycles = async (application: any) => {
    ...
    const ok = await call({ action: "update-application-cycles", applicationId: application.id, studentId: application.student_id, cycleIds: editingApplicationCycles, cycleScopes: Object.fromEntries(editingApplicationCycles.map(id=>[id,editingApplicationScopes[id]??"FULL"])) });
    if (ok) { setEditingApplicationId(""); setEditingApplicationCycles([]); setEditingApplicationScopes({}); }
  };
  ...
  return <section className="pb-admin">
    ...
    {mode==="payments"&&<article className="pb-apps"><div className="apps-title"><div><small className="step">최근 10회 참가 일정</small><h3>학부모 신청 내역</h3></div><span>신청한 날짜만 표시 · 관리자 날짜 변경 가능</span></div>{visibleApplications.map((a: any) => {
      ...
      const savedScopes = a.selected_cycle_scopes ?? {};
      const cycleText = selectedCycles.map((c: any) => `${day(c.start_date)} ${scopeLetter(savedScopes[String(c.id)]??a.scope_code)}`).join(", ") || "선택한 최근 일정 없음";
      ...
      return <div key={a.id} className={isCancelled ? "cancelled application-row" : "application-row"}><span className="app-student" style={{minWidth:300}}><small>{status[a.status] ?? a.status}</small><b>{a.student_name} · {a.school} {a.grade}</b><em>{a.parent_name} 학부모 · {won(Number(a.charged_price ?? batch?.price ?? 0))}원 · {paymentLabel(a.payment_method)}</em></span><span className="chosen-cycles" style={{display:"flex",flexDirection:"row",gap:6,flexWrap:"wrap",alignItems:"center",flex:"1 1 420px"}}>{selectedCycles.map((c: any) => <i key={c.id} style={{display:"inline-flex",whiteSpace:"nowrap",fontStyle:"normal",padding:"6px 10px",borderRadius:16,background:"#edf5ef",color:"#285e39",fontSize:11,fontWeight:800}}>{day(c.start_date)} · {scopeLetter(savedScopes[String(c.id)]??a.scope_code)}</i>)}{!selectedCycles.length ? <i className="empty-cycle" style={{display:"inline-flex",whiteSpace:"nowrap",fontStyle:"normal",padding:"6px 10px",borderRadius:16,background:"#f6f1e9",color:"#986621",fontSize:11,fontWeight:800}}>최근 10회 내 선택 일정 없음</i> : null}</span><span className="pb-link" style={{whiteSpace:"nowrap"}}>
      ...
      </span>{editingApplication ? <FinderCycles cycles={recentCycles} selected={editingApplicationCycles} scopes={editingApplicationScopes} count={purchasedCount} busy={!!busy} onToggle={toggleApplicationCycle} onScope={(id,scope)=>setEditingApplicationScopes(current=>({...current,[id]:scope}))} onSave={() => void saveApplicationCycles(a)} /> : null}</div>;
