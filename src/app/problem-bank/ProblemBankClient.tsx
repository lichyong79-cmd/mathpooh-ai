...
function normalizeDifficultyLegacy(value: unknown, dna?: any) { return normalizeProblemDifficulty(value, dna, ""); }
...
export default function ProblemBankClient() {
  ...
  // v164: 과목은 표준 6과목으로만 묶어서 보여준다. (표기 흔들림 때문에 과목이 갈라지던 문제 해결)
  const subjects = useMemo(() => {
    const present = new Set(items.map((item) => canonicalSubject(item.subject)));
    return [...SUBJECTS.filter((value) => present.has(value)), ...(present.has("미분류") ? ["미분류"] : [])];
  }, [items]);
  ...
  const difficultyQuickCounts = useMemo(() => {
    const visibleItems = items.filter((item) => !(item.problem_dna as any)?.errorReview?.open);
    return DIFFICULTY_SCALE.map((level) => ({
      ...level,
      count: visibleItems.filter((item) => item.difficulty === level.value).length,
    }));
  }, [items]);
  const visibleDifficultyTotal = useMemo(
    () => difficultyQuickCounts.reduce((sum, level) => sum + level.count, 0),
    [difficultyQuickCounts],
  );
  ...
  const dashboard = useMemo(() => {
    ...
    return {
      ...
      subjects: countBy((item) => canonicalSubject(item.subject)),
      units: countBy((item) => item.unit).slice(0, 5),
      difficulties: [...DIFFICULTY_SCALE.map(({value}) => [value, items.filter((item) => item.difficulty === value).length] as const), ["", items.filter((item)=>!item.difficulty).length] as const],
    };
  }, [items]);
  ...
  return (
    ...
    <main className="bank-page">
      ...
      <section className="bank-dashboard">
        ...
        <div className="bank-dashboard-detail">
          <article className="bank-chart-card">
            ...
            <div className="bank-bars">{dashboard.subjects.map(([label, value]) => <div key={label} className="bank-bar-row"><span>{label}</span><div><i style={{ width: `${dashboardPercent(value)}%` }} /></div><b>{value}</b></div>)}</div>
          </article>
          <article className="bank-chart-card">
            <div className="bank-card-title"><strong>난이도 분포</strong><span>SOS 8단계 + 미분류</span></div>
            <div className="difficulty-overview">{dashboard.difficulties.map(([level, value]) => <div key={level}><span>{level ? difficultyLabel(level) : "미분류"}</span><strong>{value}</strong><small>{dashboardPercent(value)}%</small></div>)}</div>
          </article>
          <article className="bank-chart-card">
      ...
      <section className="difficulty-quick-filter">
        <div className="difficulty-quick-head">
          <div><strong>난이도별 문제 보기</strong><span>누르면 해당 난이도 문항만 바로 표시됩니다.</span></div>
          <b>{difficulty === "전체" ? visibleDifficultyTotal.toLocaleString() : filtered.length.toLocaleString()}문항</b>
        </div>
        <div className="difficulty-quick-buttons">
          <button type="button" className={difficulty === "전체" ? "active" : ""} onClick={() => setDifficulty("전체")}>
            <span>전체</span><strong>{visibleDifficultyTotal.toLocaleString()}</strong>
          </button>
          {difficultyQuickCounts.map((level) => (
            <button
              key={level.value}
              type="button"
              className={difficulty === level.value ? `active level-${level.value}` : `level-${level.value}`}
              onClick={() => setDifficulty(level.value)}
            >
              <span>{level.value} · {level.label}</span><strong>{level.count.toLocaleString()}</strong>
            </button>
          ))}
          <button type="button" className={difficulty === "적4이상" ? "active high" : "high"} onClick={() => setDifficulty("적4이상")}>
            <span>적4 이상</span><strong>{difficultyQuickCounts.filter((level) => Number(level.value) >= 5).reduce((sum, level) => sum + level.count, 0).toLocaleString()}</strong>
          </button>
        </div>
      </section>
        ...
        <aside className="problem-list">
          ...
          {loading ? <MATHPOOHLoader title="문제은행 불러오는 중" detail="문항 이미지, 난이도, DNA와 해설 연결정보를 준비하고 있습니다." kind="loading" audience="admin"/> : filtered.length === 0 ? <div className="empty">조건에 맞는 문항이 없습니다.</div> : filtered.map((item) => (
            <button key={item.id} type="button" className={`problem-row ${selectedId === item.id ? "selected" : ""}`} onClick={() => setSelectedId(item.id)}>
              ...
              <span className={`difficulty-chip difficulty-${item.difficulty || "unknown"}`}>{difficultyLabel(item.difficulty)}</span>
            </button>
          ))}
        </aside>
      ...
      <style jsx>{`
        ...
        @media(max-width:1400px){.difficulty-quick-buttons{grid-template-columns:repeat(5,1fr)}.bank-kpi-grid{grid-template-columns:repeat(3,1fr)}.bank-dashboard-detail{grid-template-columns:1fr 1fr}.bank-layout{grid-template-columns:340px minmax(0,1fr)}.problem-detail{grid-template-columns:minmax(450px,1fr) minmax(390px,.85fr)}}@media(max-width:900px){.bank-kpi-grid{grid-template-columns:repeat(2,1fr)}.bank-dashboard-detail{grid-template-columns:1fr}.filter-bar{display:grid;grid-template-columns:1fr 1fr}.filter-bar label,.filter-bar label.filter-search,.filter-bar label:nth-child(2){min-width:0}.filter-bar label.filter-search,.filter-bar label:nth-child(2){grid-column:1/-1}.filter-reset{width:100%}}@media(max-width:560px){.filter-bar{grid-template-columns:1fr}.filter-bar label.filter-search,.filter-bar label:nth-child(2){grid-column:auto}}
      `}</style>
    </main>
    </AdminPortalShell>
  );
