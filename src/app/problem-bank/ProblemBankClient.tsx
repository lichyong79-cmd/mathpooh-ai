...
function normalizeDifficultyLegacy(value: unknown, dna?: any) { return normalizeProblemDifficulty(value, dna, "2"); }
    ...
    <main className="bank-page">
      ...
      <section className="bank-dashboard">
        ...
        <div className="bank-dashboard-detail">
          ...
          <article className="bank-chart-card">
            <div className="bank-card-title"><strong>난이도 분포</strong><span>SOS 8단계</span></div>
            <div className="difficulty-overview">{dashboard.difficulties.map(([level, value]) => <div key={level}><span>{difficultyLabel(level)}</span><strong>{value}</strong><small>{dashboardPercent(value)}%</small></div>)}</div>
          </article>
          <article className="bank-chart-card">
      ...
      <section className="filter-bar">
        ...
        <label><span>난이도</span><select value={difficulty} onChange={(event) => setDifficulty(event.target.value)}><option value="전체">전체</option>{difficulties.map((value) => <option key={value} value={value}>{difficultyLabel(value)}</option>)}</select></label>
          ...
          {!selected ? <div className="empty large">왼쪽에서 문항을 선택해 주세요.</div> : <>
            ...
            <form className="edit-panel" onSubmit={(event) => { event.preventDefault(); void save(); }}>
              ...
              {detailTab === "basic" ? <div className="edit-grid">
                ...
                <label><span>난이도</span><select value={draft.difficulty} onChange={(event) => setDraft({ ...draft, difficulty: event.target.value })}>{DIFFICULTY_SCALE.map((d)=><option key={d.value} value={d.value}>{d.label}</option>)}</select></label>
