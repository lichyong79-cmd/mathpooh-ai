...
function buildAnchorCrop(pageCanvas: HTMLCanvasElement, anchor: QuestionAnchor): CanonicalCrop {
  ...
  if (anchor.nextTopPct === null) {
    ...
    const climbed = climbToBlockTop(ink, nextRow, joinGap);
    // 위로 거슬러 올라가는 양을 제한한다. 분수 분자 정도(약 2%)면 충분하고,
    // 그 이상 올라가면 앞 문항의 선택지까지 삼켜 버린다.
    const climbFloor = nextRow - Math.round(pageH * 0.02);
    const nextTop = climbed === null ? null : Math.max(climbed, climbFloor);
...
export default function AnalysisWorkspacePage() {
  ...
  const activeAnchor = activeQuestion
    ? anchors?.byQuestionNo.get(Number(activeQuestion.question_no)) ?? null
    : null;
        ...
        <section className={`ai-health ${anchorBusy ? "idle" : anchors?.hasTextLayer ? "ok" : "fail"}`}>
          <div>
            ...
            <div>
              ...
              {!anchorBusy && anchors?.hasTextLayer ? (
                <small className="anchor-detail">
                  {`${anchors.diagnostics.pageCount}쪽 · ${anchors.diagnostics.columnsPerPage.join("/")}단`}
                  {` · 확정 ${anchors.diagnostics.detected.join(",") || "없음"}`}
                  {anchors.diagnostics.rejected.length
                    ? ` · 탈락 ${anchors.diagnostics.rejected.join(",")}`
                    : ""}
                  {activeAnchor
                    ? ` · 현재 ${activeAnchor.questionNo}번: ${activeAnchor.page}쪽 ${activeAnchor.column + 1}단 상단 ${activeAnchor.topPct.toFixed(1)}% 다음 ${activeAnchor.nextTopPct === null ? "없음(단 끝)" : `${activeAnchor.nextTopPct.toFixed(1)}%`}`
                    : ""}
                </small>
              ) : null}
            </div>
