...
export default function ProblemBankPage() {
  ...
  const loadProblems = useCallback(async () => {
    ...
    try {
      const fields = [
        ...
        "source_name", "confidence", "status", "created_at", "updated_at",
      ].join(",");
  ...
  useEffect(() => {
    ...
    void (async () => {
      try {
        const response = await fetch(`/api/source-files/${selected.source_file_id}/signed-urls`, { cache: "no-store" });
        const result = await response.json() as { success?: boolean; examUrl?: string | null; solutionUrl?: string | null; message?: string };
        if (!response.ok || !result.success) throw new Error(result.message || "PDF 주소를 불러오지 못했습니다.");
        setExamUrl(result.examUrl ?? null);
        setSolutionUrl(result.solutionUrl ?? null);
      } catch {
        setExamUrl(null);
        setSolutionUrl(null);
      }
  ...
  const shownPdf = viewer === "exam" ? examUrl : solutionUrl;
            ...
            <div className="pdf-panel">
              <div className="pdf-head">
                ...
                <div className="pdf-tabs">
                  <button className={viewer === "exam" ? "active" : ""} onClick={() => setViewer("exam")}>시험지</button>
                  <button className={viewer === "solution" ? "active" : ""} onClick={() => setViewer("solution")}>해설지</button>
                </div>
              </div>
              <div className="pdf-viewer">{shownPdf ? <iframe title={`${viewer} PDF`} src={`${shownPdf}#page=1&view=FitH`} /> : <span>표시할 PDF가 없습니다.</span>}</div>
            </div>
