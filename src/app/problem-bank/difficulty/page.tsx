...
function norm(v: unknown, dna?: any) {
  return normalizeProblemDifficulty(v, dna, "");
}
...
function bandLabel(value?: string) {
  const map: Record<string, string> = {
    ...
    four_hard: "어4",
    semi_killer: "준킬러",
    semi_killer_easy: "준킬러",
    semi_killer_hard: "준킬러",
    killer: "킬러",
  };
...
function failureTypeLabel(value: unknown) {
  ...
  return (
    (
      {
        timeout: "시간초과",
        http_429: "API 사용량/속도 제한",
        http_4xx: "API 요청 오류",
        http_5xx: "AI 서버 오류",
        response_json_parse: "응답 형식 오류",
        incomplete_max_output_tokens: "출력 토큰 한도",
        incomplete_content_filter: "콘텐츠 필터",
        incomplete_other: "미완료 응답",
        empty_response: "빈 응답",
        structured_json_parse: "구조화 JSON 파싱 실패",
        unknown: "원인 미확정",
...
function TestProblemImage({
    ...
    void (async () => {
      try {
        const response = await fetch(
          `/api/problem-bank/questions/${encodeURIComponent(problemId)}/image`,
          { cache: "no-store" },
        );
        ...
        if (alive)
          setUrl(
            response.ok && result.success ? (result.imageUrl ?? null) : null,
          );
      } catch {
  ...
  if (!url)
    return <div style={emptyStyle}>문항 이미지를 불러오지 못했습니다.</div>;
  return (
    <div
...
export default function DifficultyManagementPage() {
  ...
  async function queueAction(action: "enqueue" | "retry" | "clear") {
    ...
    const label =
      action === "enqueue"
        ? "AI 미검증 문항을 재판정 대기열에 넣습니다."
        : action === "retry"
          ? "실패한 문항을 다시 대기열에 넣습니다."
    ...
    try {
      ...
      const data = await res.json().catch(() => ({}));
      if (!data?.success)
        throw new Error(data?.message || "큐 처리에 실패했습니다.");
      if (action === "enqueue")
        setMessage(
          `대기열에 ${data.inserted}문항을 넣었습니다. (대상 ${data.candidates}문항 중 이미 등록된 건 제외)`,
        );
      else if (action === "retry")
        setMessage(`실패 ${data.requeued}문항을 다시 대기열에 넣었습니다.`);
  ...
  const filtered = useMemo(
    () =>
      items.filter((x) => {
        if (difficulty === "미분류" && norm(x.difficulty, x.problem_dna))
          return false;
        // SOS275: AI 검증이 필요한 문항만 골라 작업할 수 있게 한다.
        if (difficulty === "AI미검증" && difficultyAiJudged(x.problem_dna))
          return false;
        if (
          difficulty !== "전체" &&
          difficulty !== "미분류" &&
          difficulty !== "AI미검증" &&
          norm(x.difficulty, x.problem_dna) !== difficulty
        )
          return false;
        if (subject !== "전체" && canonicalSubject(x.subject) !== subject)
          return false;
  ...
  const counts = useMemo(
    () =>
      D.map(
        (d) =>
          items.filter((x) => norm(x.difficulty, x.problem_dna) === d).length,
      ),
    [items],
  );
  const unclassifiedCount = useMemo(
    () => items.filter((x) => !norm(x.difficulty, x.problem_dna)).length,
    [items],
  );
  const reviewCount = useMemo(
    () =>
      items.filter((x) =>
        problemDifficultyNeedsReview(x.difficulty, x.problem_dna),
      ).length,
    [items],
  );
  // SOS274: scale_version은 8단계 전환 SQL이 옛 1~5 환산값에도 도장을 찍어놨기 때문에
  // "검증됨"의 근거가 되지 못한다. 실제 AI 재판정 흔적(ai_regrade_version)으로 센다.
  const notJudgedCount = useMemo(
    () => items.filter((x) => !difficultyAiJudged(x.problem_dna)).length,
    [items],
  );
  const legacyStampCount = useMemo(
    () =>
      items.filter(
        (x) =>
          String(x.problem_dna?.difficulty?.scale_version ?? "") !==
          DIFFICULTY_SCALE_VERSION,
      ).length,
    [items],
  );
  const sampleSummary = useMemo(() => {
    ...
    const reviewRows = unique.filter(
      (x) => x.result?.ok && !!x.result?.reviewRequired,
    );
    const unclassifiedRows = unique.filter(
      (x) =>
        x.result?.ok &&
        !x.result?.reviewRequired &&
        (x.result?.decision === "unclassified" || !x.result?.difficulty),
    );
    const graded = unique.filter(
      (x) =>
        x.result?.ok &&
        !x.result?.reviewRequired &&
        x.result?.decision !== "unclassified" &&
        x.result?.difficulty,
    );
    const changedRows = graded.filter(
      (x) => x.before !== String(x.result?.difficulty),
    );
    const keptRows = graded.filter(
      (x) => x.before === String(x.result?.difficulty),
    );
  ...
  const fullPreviewSummary = useMemo(() => {
    const unique = [
      ...new Map(fullPreviewResults.map((x) => [x.id, x])).values(),
    ];
    const reviewRows = unique.filter(
      (x) => x.result?.ok && !!x.result?.reviewRequired,
    );
  ...
  async function changeDifficulty(id: string, value: string) {
    ...
    setError("");
    const res = await fetch(
      `${config.url}/rest/v1/problem_bank_questions?id=eq.${encodeURIComponent(id)}`,
      {
        method: "PATCH",
        headers: {
          ...(await authHeaders()),
    ...
    const saved = (await res.json().catch(() => [])) as Array<{
      difficulty?: string | number;
      problem_dna?: any;
    }>;
    const confirmed = norm(
      saved?.[0]?.difficulty ?? value,
      saved?.[0]?.problem_dna ?? dna,
    );
    const confirmedDna = saved?.[0]?.problem_dna ?? dna;
    setItems((prev) =>
      prev.map((x) =>
        x.id === id
          ? { ...x, difficulty: confirmed, problem_dna: confirmedDna }
          : x,
      ),
    );
    setTestResults((prev) =>
    ...
    setMessage(
      `${target.question_no}번 난이도 ${difficultyLabel(previous)} → ${difficultyLabel(confirmed)} 저장 완료`,
    );
    setSavingId(null);
  }
...
진행할까요?`,
    ...
    try {
      while (true) {
        const res = await fetch(
          "/api/problem-bank/recalculate-difficulty-from-dna",
          {
            method: "POST",
            headers: { "Content-Type": "application/json" },
        ...
        if (!res.ok || data?.success !== true)
          throw new Error(data?.message || `DNA 재계산 실패 (${res.status})`);
        ...
        setProgress({
          mode: "anomaly",
          done: Math.min(offset, items.length),
          total: items.length,
          ok,
          fail,
        });
        setMessage(
          `DNA 난이도 재계산 중 · ${Math.min(offset, items.length)}/${items.length} · 변경 ${ok} · 관리자확정 보존 ${fixedSkipped} · DNA없음 ${noDna} · 실패 ${fail}`,
        );
        if (data.done === true) break;
      }
      await load();
      setMessage(
        `DNA 난이도 재계산 완료 · 변경 ${ok} · 관리자확정 보존 ${fixedSkipped} · DNA없음 ${noDna}${fail ? ` · 실패 ${fail}` : ""} · AI 호출 0회`,
      );
    } catch (e) {
      setError(
        e instanceof Error ? e.message : "DNA 난이도 재계산에 실패했습니다.",
      );
    } finally {
      setRunning(false);
      setProgress(null);
  ...
  function buildReferenceIds() {
    const validFixed = items.filter(
      (x) =>
        x.problem_dna?.difficulty?.admin_fixed === true &&
        x.problem_dna?.difficulty?.scale_version === DIFFICULTY_SCALE_VERSION,
    );
    return DIFFICULTY_SCALE.flatMap((scale) =>
      validFixed
  ...
  function buildBalancedSample(perGrade = 4) {
    ...
    for (const scale of DIFFICULTY_SCALE) {
      const pool = candidates.filter(
        (x) => norm(x.difficulty, x.problem_dna) === scale.value,
      );
      chosen.push(...pool.slice(0, perGrade));
    }
    chosen.push(
      ...candidates
        .filter((x) => !norm(x.difficulty, x.problem_dna))
        .slice(0, 8),
    );
    // 3점 과밀 구간을 별도로 더 검사한다.
    chosen.push(
      ...candidates
        .filter((x) => norm(x.difficulty, x.problem_dna) === "2")
        .slice(perGrade, perGrade + 12),
    );
  ...
  async function runSampleRecheck() {
    ...
    if (!targets.length) {
      setMessage("표본 재검증 대상이 없습니다.");
      return;
    }
    if (
      !window.confirm(
        `새 SOS245 엔진이 ${targets.length}문항을 실제로 다시 풀고 난이도를 검증합니다.\n\nDB 난이도는 변경하지 않습니다.\n결과를 먼저 비교해 본 뒤 전체 적용 여부를 결정할 수 있습니다.\n\n진행할까요?`,
      )
    )
      return;
    setRunning(true);
      ...
      for (let i = 0; i < targets.length; i += 8) {
        ...
        for (const target of batch) {
          ...
          if (result?.ok || result?.success) {
            ...
            rows.push({
              ...target,
              before: norm(target.difficulty, target.problem_dna),
        ...
        setProgress({ mode: "sample", done, total: targets.length, ok, fail });
        setMessage(
          `표본 재검증 중 · ${done}/${targets.length} · 성공 ${ok} · 미판정/실패 ${fail}`,
        );
      }
      setMessage(
        `SOS249 표본 재검증 완료 · ${targets.length}문항. 아래에서 기존↔신규 판정과 미판정/검토필요를 확인하세요. DB 난이도는 아직 변경하지 않았습니다.`,
      );
    } catch (e) {
      setError(e instanceof Error ? e.message : "표본 재검증에 실패했습니다.");
    } finally {
      setRunning(false);
      setProgress(null);
    }
  ...
  async function retryVerification(problemId: string) {
    ...
    const target =
      testResults.find((x) => x.id === problemId) ??
      items.find((x) => x.id === problemId);
    if (!target) return;
    setRetryingId(problemId);
    setError("");
    setMessage(`${target.question_no}번 검증을 다시 시도합니다...`);
    try {
      const referenceIds = buildReferenceIds();
      const res = await fetch("/api/problem-bank/regrade-difficulty-batch", {
        method: "POST",
      ...
      const normalizedResult =
        result?.ok || result?.success
          ...
          : {
              ...
              ok: false,
              message:
                result?.message || data?.message || `검증 실패 (${res.status})`,
              failureType:
                result?.failureType || data?.failureType || "unknown",
              failureStage:
                result?.failureStage || data?.failureStage || "unknown",
              failureDetail: result?.failureDetail || data?.failureDetail || "",
            };
      setTestResults((prev) =>
        prev.map((row) =>
          row.id === problemId ? { ...row, result: normalizedResult } : row,
        ),
      );
      if (normalizedResult.ok)
        setMessage(
          `${target.question_no}번 재검증 완료 · ${normalizedResult.decision === "unclassified" || !normalizedResult.difficulty ? "미판정" : difficultyLabel(normalizedResult.difficulty)}`,
        );
      else
        setMessage(
          `${target.question_no}번 재검증도 실패했습니다. 기존 난이도는 그대로 유지됩니다.`,
        );
    } catch (e) {
  ...
  async function retryAllFailures() {
    if (running || retryingId) return;
    const failedIds = [
      ...new Set(testResults.filter((x) => !x.result?.ok).map((x) => x.id)),
    ];
    if (!failedIds.length) {
      setMessage("재검증할 실패 문항이 없습니다.");
      return;
    }
    setRunning(true);
    setError("");
    setMessage(`검증실패 ${failedIds.length}문항을 다시 검증합니다...`);
    const referenceIds = buildReferenceIds();
    ...
    try {
      for (let i = 0; i < failedIds.length; i += 4) {
        ...
        const res = await fetch("/api/problem-bank/regrade-difficulty-batch", {
          method: "POST",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify({ problemIds: ids, dryRun: true, referenceIds }),
        });
        const data = await res.json().catch(() => ({}));
        const results = Array.isArray(data.results) ? data.results : [];
        const map = new Map(results.map((r: any) => [String(r.problemId), r]));
            ...
            r?.ok || r?.success
              ...
              : {
                  ...
                  message:
                    ...
                    data?.message ||
                    `재검증 실패 (${res.status})`,
                  failureType: r?.failureType || data?.failureType || "unknown",
                  failureStage:
                    r?.failureStage || data?.failureStage || "unknown",
                  failureDetail: r?.failureDetail || data?.failureDetail || "",
                };
      ...
      setTestResults((prev) =>
        prev.map((row) =>
          replacements.has(row.id)
            ? { ...row, result: replacements.get(row.id) }
            : row,
        ),
      );
      const recovered = failedIds.filter(
        (id) => replacements.get(id)?.ok,
      ).length;
      const stillFailed = failedIds.length - recovered;
      setMessage(
        `검증실패 재시도 완료 · 복구 ${recovered} · 여전히 실패 ${stillFailed}. 실패 문항은 기존 난이도를 유지합니다.`,
      );
    } catch (e) {
      setError(
        e instanceof Error ? e.message : "실패 문항 재검증에 실패했습니다.",
      );
    } finally {
      setRunning(false);
    }
  ...
  async function runVerifiedFullPreview() {
    ...
    if (!targets.length) {
      setMessage("전체 재검증 대상이 없습니다.");
      return;
    }
    if (
      !window.confirm(`SOS248 검증 엔진으로 ${targets.length}문항의 전체 재판정 '미리보기'를 계산합니다.
...
AI 호출량이 많고 시간이 걸릴 수 있습니다. 미리보기를 시작할까요?`)
    )
      return;
    ...
    try {
      for (let i = 0; i < targets.length; i += 8) {
        ...
        for (const target of batch) {
          ...
          if (normalized.ok) {
            ...
            if (
              ...
              normalized.decision === "unclassified" ||
        ...
        setMessage(
          `전체 난이도 미리보기 계산 중 · ${done}/${targets.length} · 정상 ${ok} · 검토/미판정 ${review} · 실패 ${fail} · DB 변경 0건`,
        );
      }
      setFullPreviewResults([...rows]);
      setPreviewGeneratedAt(new Date().toISOString());
      setMessage(
        `SOS248 전체 미리보기 완료 · ${targets.length}문항 · DB 변경 0건. 아래 예상 분포와 이동표를 확인한 뒤 최종 적용하세요.`,
      );
    } catch (e) {
      setError(
        e instanceof Error ? e.message : "전체 재판정 미리보기에 실패했습니다.",
      );
    } finally {
      setRunning(false);
      setProgress(null);
  ...
  async function applyFullPreview() {
    if (running) return;
    const eligible = fullPreviewResults.filter(
      (x) =>
        x.result?.ok &&
        x.result?.decision === "graded" &&
        !!x.result?.difficulty &&
    ...
    if (!eligible.length) {
      setMessage("적용 가능한 미리보기 결과가 없습니다.");
      return;
    }
    ...
    if (
      !window.confirm(`미리보기에서 검증 통과한 ${eligible.length}문항을 DB에 적용합니다.
...
최종 적용할까요?`)
    ...
    try {
      for (let i = 0; i < eligible.length; i += 100) {
        ...
        const res = await fetch(
          "/api/problem-bank/apply-difficulty-preview-batch",
          {
            method: "POST",
            headers: { "Content-Type": "application/json" },
            body: JSON.stringify({
        ...
        if (!res.ok || data?.success !== true)
          throw new Error(
            data?.message || `미리보기 적용 실패 (${res.status})`,
          );
        ...
        setMessage(
          `미리보기 결과 DB 적용 중 · ${done}/${eligible.length} · 적용 ${applied} · 중간변경으로 건너뜀 ${stale} · 실패 ${failed}`,
        );
      }
      await load();
      setFullPreviewResults([]);
      setPreviewGeneratedAt("");
      setMessage(
        `SOS248 난이도 최종 적용 완료 · 적용 ${applied} · 중간변경 건너뜀 ${stale} · 실패 ${failed}. 검토필요/미판정/실패 문항은 기존 난이도를 유지했습니다.`,
      );
    } catch (e) {
      setError(
        e instanceof Error ? e.message : "미리보기 결과 적용에 실패했습니다.",
      );
    } finally {
      setRunning(false);
      setProgress(null);
  ...
  async function runAnomalyReview() {
    ...
    const ranked = items
      ...
      .map((x) => {
        ...
        const confidence = Number(
          d.ai_regrade_confidence ?? x.problem_dna?.confidence ?? 1,
        );
        // v164: 문항 근거(개념·조건해석·발상·계산·시간)만으로 계산한 난이도.
        // 밴드만 보고 3점으로 몰려 등록된 문항을 찾아내는 핵심 신호다.
        const evidence =
          d && Object.keys(d).length ? String(evidenceDifficultyLevel(d)) : "";
        const evidenceGap =
          evidence && current
            ? Math.abs(Number(evidence) - Number(current))
            : 0;
        let suspicion = 0;
        if (bandValue && bandValue !== current) suspicion += 100;
        if (/^[1-8]$/.test(finalValue) && finalValue !== current)
          suspicion += 90;
        ...
        if (Number.isFinite(confidence) && confidence < 0.72)
          suspicion += 70 + Math.round((0.72 - confidence) * 100);
    ...
    const fallback = ranked
      .filter((r) => r.suspicion === 0)
      .slice(0, 40)
      .map((r) => r.x);
    const targets = [...suspicious, ...fallback].slice(0, 120);
    if (!targets.length) {
      setMessage("AI 이상 난이도 검토 대상이 없습니다.");
      return;
    }

    if (
      !window.confirm(
        `AI가 ${targets.length}문항을 2차 검토하고, 현재 난이도와 다르게 판단한 문항만 보여줍니다. DB는 자동 변경하지 않습니다. 진행할까요?`,
      )
    )
      return;
    ...
    let ok = 0,
      fail = 0;
    try {
      // SOS304: AI 재풀이는 문항당 최대 180초가 걸릴 수 있다. 20문항을 한 요청에
      // 넣으면 Vercel 300초 제한을 넘으므로, 서버 동시 처리 수와 같은 4문항씩 보낸다.
      // 일시적인 네트워크 종료는 같은 묶음만 한 번 재시도하고 전체 검토는 계속한다.
      const batchSize = 4;
      for (let i = 0; i < targets.length; i += batchSize) {
        ...
        for (let attempt = 1; attempt <= 2; attempt++) {
          try {
            const res = await fetch(
              "/api/problem-bank/regrade-difficulty-batch",
              {
                method: "POST",
                headers: { "Content-Type": "application/json" },
                body: JSON.stringify({
            ...
            if (!res.ok || data?.success !== true)
              throw new Error(
                data?.message || `AI 검토 요청 실패 (${res.status})`,
              );
            batchError = "";
            break;
          } catch (e) {
            batchError = e instanceof Error ? e.message : "AI 검토 통신 실패";
            if (attempt < 2) {
              setMessage(
                `AI 검토 통신 재시도 중 · ${i + 1}~${i + batch.length}번 묶음`,
              );
        ...
        if (batchError || !data) {
          ...
          setMessage(
            `AI 이상 난이도 검토 계속 진행 · ${done}/${targets.length} · 이상 의심 ${anomalies.length} · 검토 실패 ${fail}`,
          );
          continue;
        }
        const results = Array.isArray(data.results) ? data.results : [];
        ...
        for (const target of batch) {
          ...
          if (result?.ok || result?.success) {
            ...
            if (
              proposed &&
              proposed !== current &&
              Number(result.confidence ?? 0) >= 0.6
            ) {
              anomalies.push({
                ...target,
                before: current,
        ...
        setMessage(
          `AI 이상 난이도 검토 중 · ${done}/${targets.length} · 이상 의심 ${anomalies.length} · 검토 실패 ${fail}`,
        );
      }
      setMessage(
        `AI 이상 난이도 검토 완료 · ${targets.length}문항 확인 · 이상 의심 ${anomalies.length}문항${fail ? ` · 실패 ${fail}` : ""}. 자동 변경은 하지 않았습니다.`,
      );
    } catch (e) {
      setError(
        e instanceof Error ? e.message : "AI 이상 난이도 검토에 실패했습니다.",
      );
    } finally {
      setRunning(false);
      setProgress(null);
    ...
    <AdminPortalShell current="sos-difficulty">
      <main className="difficulty-page">
        <div className="difficulty-wrap">
          <div className="difficulty-header">
            <div>
              ...
              <h1>난이도 관리</h1>
              <p>
                AI가 문제를 먼저 재풀이한 뒤 8단계 난이도를 판정합니다. 관리자가
                확정한 난이도는 자동으로 덮어쓰지 않습니다. <b>AI 미검증</b>은
                저장된 난이도가 AI 재풀이로 확정된 값이 아닌 문항 수입니다. DNA
                점수로 계산한 추정치, 옛 1~5단계 환산값, AI 판정 뒤에 보조
                계산이 덮어쓴 문항이 모두 포함됩니다. 등급은 나왔으나 정답
                대조나 신뢰도에 확인이 필요한 문항은 미판정이 아니라{" "}
                <b>검토필요</b>로 분리되어 여기서 직접 확정할 수 있습니다.
              </p>
            </div>
            <div className="header-buttons">
              <button
                onClick={() => (location.href = "/problem-bank/barometer")}
              >
                학생·문항 바로미터
              </button>
              <button
              ...
              <button onClick={() => (location.href = "/problem-bank")}>
                SOS 문제은행
              </button>
            </div>
          </div>
          ...
          {progress && (
            <div className="progress-card">
              <div className="progress-copy">
                <b>
                  {progress.mode === "sample"
                    ? "표본 재풀이 검증 중입니다..."
                    : progress.mode === "full"
                      ? "전체 난이도 미리보기/적용 작업 중입니다..."
                      : "AI 이상 난이도 검토 중입니다..."}
                </b>
                <span>
                  {progress.done}/{progress.total} · 성공 {progress.ok} · 실패{" "}
                  {progress.fail}
                </span>
              </div>
          ...
          <section className="queue-card">
            <div className="queue-head">
              <div>
                <b>백그라운드 난이도 재판정</b>
                <span>
                  서버가 10분마다 몇 문항씩 처리합니다. 이 화면을 닫아도 계속
                  진행됩니다.
                </span>
              </div>
              <strong>
                {queue ? `${queue.finished ?? 0} / ${queue.total ?? 0}` : "-"}
              </strong>
            </div>
            <div className="queue-track">
              <i style={{ width: `${queue?.percent ?? 0}%` }} />
            </div>
            <div className="queue-stats">
              <div>
                <b>{queue?.counts?.QUEUED ?? 0}</b>
                <span>대기</span>
              </div>
              <div>
                <b>{queue?.counts?.RUNNING ?? 0}</b>
                <span>처리 중</span>
              </div>
              <div>
                <b>{queue?.counts?.DONE ?? 0}</b>
                <span>완료</span>
              </div>
              <div className="bad">
                <b>{queue?.counts?.FAILED ?? 0}</b>
                <span>실패</span>
              </div>
              <div>
            ...
            <div className="queue-actions">
              <button
                disabled={queueBusy}
                onClick={() => void queueAction("enqueue")}
                className="primary"
              >
                AI 미검증 문항 대기열에 넣기
              </button>
              <button
                disabled={queueBusy || !queue?.counts?.FAILED}
                onClick={() => void queueAction("retry")}
              >
                실패 {queue?.counts?.FAILED ?? 0}건 재시도
              </button>
              <button
                disabled={queueBusy || !queue?.counts?.QUEUED}
                onClick={() => void queueAction("clear")}
                className="legacy-action"
              >
                대기 비우기
              </button>
              <button disabled={queueBusy} onClick={() => void loadQueue()}>
                새로고침
              </button>
            </div>
            <p className="queue-note">
              처리 순서: 미분류 → 3점·어3 → 나머지. 관리자 확정 문항과 AI 확정
              문항은 대상에서 제외됩니다.
            </p>
          </section>
          ...
          <div className="filter-bar">
            <input
              ...
              placeholder="문항번호·시험지·단원·유형·출처·DNA 검색"
            />
            <select
              value={subject}
            ...
            >
              <option>전체</option>
              {DIFFICULTY_SCALE.map((x) => (
                <option key={x.value} value={x.value}>
                  {x.label}
                </option>
              ))}
              <option value="미분류">미분류</option>
              <option value="AI미검증">AI 미검증</option>
            </select>
            <button
              disabled={running}
              onClick={runSampleRecheck}
              className="primary"
            >
              ① 표본 재풀이 검증
            </button>
            {sampleSummary.failed > 0 && (
              <button
                ...
                className="retry-all"
              >
                ↻ 검증실패 {sampleSummary.failed}문항 다시 검증
              </button>
            )}
            <button
              disabled={running || testResults.length === 0}
              title={
                !testResults.length
                  ? "먼저 표본 재풀이 검증을 실행하세요"
                  : sampleSummary.failed > 0
                    ? `표본에 검증실패 ${sampleSummary.failed}문항이 있습니다. 그 문항은 미리보기 대상에서 제외됩니다.`
                    : "DB를 바꾸지 않고 전체 예상 난이도를 계산합니다"
              }
              onClick={runVerifiedFullPreview}
            >
              ② 전체 재판정 미리보기
            </button>
            <button disabled={running} onClick={runAnomalyReview}>
              AI 이상 난이도 검토
            </button>
            <button
              disabled={running}
              onClick={runFullEightScaleReview}
              className="legacy-action"
            >
              DNA만 재계산(보조)
            </button>
          ...
          {fullPreviewResults.length > 0 && (
            <section className="preview-section">
              <div className="preview-head">
                <div>
                  ...
                  <span>
                    DB는 아직 변경되지 않았습니다.
                    {previewGeneratedAt
                      ? ` · 계산 ${new Date(previewGeneratedAt).toLocaleString("ko-KR")}`
                      : ""}
                  </span>
                </div>
              ...
              <div className="preview-stats">
                <div>
                  <b>{fullPreviewSummary.kept}</b>
                  <span>기준 유지</span>
                </div>
                <div>
                  <b>{fullPreviewSummary.changed}</b>
                  <span>난이도 변경 예상</span>
                </div>
                <div>
                  <b>{fullPreviewSummary.review}</b>
                  <span>검토필요 · 보존</span>
                </div>
                <div>
                  <b>{fullPreviewSummary.unclassified}</b>
                  <span>미판정 · 보존</span>
                </div>
                <div className={fullPreviewSummary.failed ? "bad" : ""}>
                  <b>{fullPreviewSummary.failed}</b>
                  <span>검증실패 · 보존</span>
                </div>
              </div>
                ...
                <div>
                  ...
                  <div className="dist-grid">
                    {DIFFICULTY_SCALE.map((d) => (
                      <span key={`next-${d.value}`}>
                        <small>{d.label}</small>
                        <b>{fullPreviewSummary.expected.get(d.value) || 0}</b>
                      </span>
                    ))}
                  </div>
                </div>
              ...
              <div className="preview-matrix">
                <b>주요 이동</b>
                <span>
                  {fullPreviewSummary.matrix
                    .slice(0, 20)
                    .map(
                      ([k, v]) =>
                        `${difficultyLabel(k.split("→")[0])}→${difficultyLabel(k.split("→")[1])} ${v}`,
                    )
                    .join(" · ") || "이동 없음"}
                </span>
              </div>
              <div className="preview-warning">
                <b>아직 DB 변경 0건</b>
                <span>
                  검토필요·미판정·실패는 최종 적용에서도 기존 난이도를
                  유지합니다. 미리보기 결과가 납득될 때만 아래 버튼을 누르세요.
                </span>
              </div>
              <button
                className="apply-preview"
                disabled={running || fullPreviewSummary.applicable === 0}
                onClick={() => void applyFullPreview()}
              >
                ③ 미리보기 검증 결과 DB에 최종 적용
              </button>
            </section>
          )}
          ...
          {testResults.length > 0 && (
            <section className="test-section">
              <div className="test-section-head">
                <div>
                  <b>SOS249 난이도 표본 재검증 결과</b>
                  <span>
                    현재값과 새 재풀이 검증 결과를 비교합니다. 미판정/검토필요는
                    자동 적용하지 않습니다.
                  </span>
                </div>
                <strong>{testResults.length}문항</strong>
              </div>
              <div className="sample-summary">
                <div>
                  <b>{sampleSummary.kept}</b>
                  <span>기준 유지</span>
                </div>
                <div>
                  <b>{sampleSummary.changed}</b>
                  <span>기준과 변경</span>
                </div>
                <div>
                  <b>{sampleSummary.unclassified}</b>
                  <span>미판정</span>
                </div>
                <div>
                  <b>{sampleSummary.review}</b>
                  <span>검토필요</span>
                </div>
                <div className="failure-summary">
                  <b>{sampleSummary.failed}</b>
                  <span>검증실패</span>
                </div>
                <div className="matrix">
                  <strong>집계</strong>
                  <span>
                    {sampleSummary.accounted}/{sampleSummary.total} ·{" "}
                    {sampleSummary.accounted === sampleSummary.total
                      ? "정상"
                      : "⚠ 중복/누락 확인"}
                  </span>
                  <strong>주요 이동</strong>
                  <span>
                    {sampleSummary.matrix
                      .slice(0, 8)
                      .map(
                        ([key, count]) =>
                          `${key.replace(/^(\d)→(\d)$/, (m, a, b) => `${difficultyLabel(a)}→${difficultyLabel(b)}`)} ${count}`,
                      )
                      .join(" · ") || "변경 없음"}
                  </span>
                </div>
              ...
              <div className="test-grid">
                {testResults.map((x) => (
                  <article
                    key={x.id}
                    className={`test-card ${x.result?.ok ? (x.before !== String(x.result.difficulty) ? "changed" : "same") : "failed"}`}
                  >
                    <div className="test-card-head">
                      ...
                      <code>{x.problem_code}</code>
                    </div>
                    <div className="test-image-wrap">
                      <TestProblemImage
                        problemId={x.id}
                        questionNo={x.question_no}
                      />
                    </div>
                    ...
                    <div className="difficulty-compare">
                      <div>
                        <small>
                          현재 저장 난이도
                          {x.before !== norm(x.difficulty, x.problem_dna)
                            ? ` · 테스트 당시 ${x.before}`
                            : ""}
                        </small>
                        <strong>
                          {difficultyLabel(norm(x.difficulty, x.problem_dna))}
                        </strong>
                      </div>
                      <span>→</span>
                      <div>
                        <small>AI 제안</small>
                        <strong
                          className={x.result?.ok ? "proposal" : "fail-text"}
                        >
                          {x.result?.ok
                            ? x.result.decision === "unclassified" ||
                              !x.result.difficulty
                              ? "미판정"
                              : difficultyLabel(x.result.difficulty)
                            : "실패"}
                        </strong>
                      </div>
                    </div>
                    {x.result?.ok ? (
                      <>
                        <div className="ai-meta">
                          <span>{bandLabel(x.result.csatDifficultyBand)}</span>
                          <span>
                            난이도 신뢰도 {confidenceLabel(x.result.confidence)}
                          </span>
                          <span>
                            재풀이{" "}
                            {x.result.solutionVerified
                              ? "검증완료"
                              : "검증필요"}
                          </span>
                          <span>
                            정답대조 {x.result.answerConsistency || "unknown"}
                          </span>
                          {x.result.reviewRequired && (
                            <span className="warn-chip">검토필요</span>
                          )}
                        </div>
                        <div className="solve-box">
                          <b>AI 독립 재풀이</b>
                          <span>산출답 {x.result.solvedAnswer || "-"}</span>
                          <span>
                            추론 {x.result.reasoningSteps ?? 0}단계 · 조건변환{" "}
                            {x.result.conditionTransformations ?? 0} · 계산부담{" "}
                            {x.result.calculationLoad ?? 0}/5 · 발상부담{" "}
                            {x.result.insightLoad ?? 0}/5
                          </span>
                          {x.result.reviewReason && (
                            <small>{x.result.reviewReason}</small>
                          )}
                        </div>
                        <div className="reason-box">
                          <b>AI 판정 근거</b>
                          <p>{x.result.reason || "판정 근거가 없습니다."}</p>
                        </div>
                        <div className="review-actions">
                          ...
                          <button
                            ...
                            disabled={
                              ...
                              !x.result.difficulty ||
                              x.result.reviewRequired ||
                              norm(x.difficulty, x.problem_dna) ===
                                String(x.result.difficulty)
                            }
                            onClick={() =>
                              void changeDifficulty(
                                x.id,
                                String(x.result?.difficulty ?? x.before),
                              )
                            }
                          >
                            {savingId === x.id
                              ...
                              : x.result.decision === "unclassified" ||
                                ...
                                : x.result.reviewRequired
                                  ...
                                  : norm(x.difficulty, x.problem_dna) ===
                                      String(x.result.difficulty)
                                    ? "현재값과 동일"
                                    : `AI 제안 ${difficultyLabel(x.result.difficulty)} 적용`}
                          </button>
                          <label>
                            <span>직접 수정</span>
                            <select
                              disabled={savingId === x.id}
                              value={norm(x.difficulty, x.problem_dna)}
                              onChange={(e) =>
                                void changeDifficulty(x.id, e.target.value)
                              }
                            >
                        ...
                        {savedDifficulty[x.id] && (
                          <div className="saved-inline">
                            ✓ DB에 {difficultyLabel(savedDifficulty[x.id])}로
                            저장됨
                          </div>
                        )}
                      </>
                      ...
                      <>
                        <div className="reason-box failure">
                          <b>검증실패 · 기존 난이도 유지</b>
                          <p>
                            {x.result?.message || "AI 판정에 실패했습니다."}
                          </p>
                          <small>
                            원인: {failureTypeLabel(x.result?.failureType)}
                            {x.result?.failureStage
                              ? ` · 단계 ${failureStageLabel(x.result.failureStage)}`
                              : ""}
                          </small>
                          {x.result?.failureDetail ? (
                            <details>
                              <summary>기술 상세</summary>
                              <pre>
                                {String(x.result.failureDetail).slice(0, 1600)}
                              </pre>
                            </details>
                          ) : null}
                        </div>
                        <div className="failure-actions">
                          <button
                            disabled={running || !!retryingId}
                            onClick={() => void retryVerification(x.id)}
                          >
                            {retryingId === x.id
                              ? "다시 검증 중..."
                              : "↻ 이 문항 다시 검증"}
        ...
        <style jsx>{`
          ...
          .difficulty-kpis strong {
            display: block;
            font-size: 28px;
            margin-top: 6px;
          }
          .difficulty-kpis .kpi-warn {
            background: #fff8ee;
            border-color: #ecd4a9;
          }
          .difficulty-kpis .kpi-review {
            background: #fff1f1;
            border-color: #ecc5c5;
          }
          .queue-card {
          ...
          .distribution-compare > div {
            border: 1px solid #e0e7e3;
            border-radius: 11px;
            padding: 12px;
          }
          .distribution-compare > div > strong {
            display: block;
            margin-bottom: 9px;
          }
          .dist-grid {
          ...
          .test-section-head span {
            font-size: 13px;
            color: #6e7a73;
          }
          .test-section-head > strong {
            color: #247a4b;
          }
          .sample-summary {
            display: grid;
            grid-template-columns: repeat(5, 110px) minmax(240px, 1fr);
            gap: 8px;
            margin-bottom: 14px;
          }
          .sample-summary > div {
          ...
          .sample-summary .failure-summary {
            background: #fff1f1;
          }
          .sample-summary .failure-summary b {
            color: #a83333;
          }
          .sample-summary .matrix {
            justify-content: center;
          }
          .sample-summary .matrix strong {
            font-size: 11px;
            color: #315f43;
          }
          .sample-summary .matrix span {
            line-height: 1.45;
          }
          .test-grid {
            display: grid;
          ...
          .test-card.changed {
            border-color: #e2bd74;
            box-shadow: inset 4px 0 0 #d49a2d;
          }
          .test-card.failed {
            border-color: #edc7c7;
            box-shadow: inset 4px 0 0 #c94d4d;
          }
          .test-card-head {
            display: flex;
            justify-content: space-between;
          ...
          .difficulty-compare > div {
            display: flex;
            align-items: center;
            justify-content: center;
            gap: 8px;
          }
          .difficulty-compare small {
            color: #748078;
          }
          .difficulty-compare strong {
            font-size: 24px;
          }
          .difficulty-compare .proposal {
            color: #1d7545;
          }
          .difficulty-compare .fail-text {
            color: #b63737;
            font-size: 16px;
          }
          .ai-meta {
          ...
          .bank-row {
            display: grid;
            grid-template-columns: 80px 150px 1.2fr 1fr 130px;
            gap: 10px;
            padding: 12px;
          ...
          } /* SOS283: 좁은 화면에서 검색창 min-width:300px가 넘쳐 가로 스크롤이 생겼다. */
          @media (max-width: 760px) {
            .filter-bar input {
              min-width: 0;
              width: 100%;
            ...
            .bank-row > span:nth-child(4),
            .bank-row.head > span:nth-child(2),
            .bank-row.head > span:nth-child(4) {
              display: none;
            }
            .progress-copy {
              flex-direction: column;
