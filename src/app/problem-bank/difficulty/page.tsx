...
export default function DifficultyManagementPage() {
  ...
  async function changeDifficulty(id:string, value:string) {
    ...
    setTestResults(prev => prev.map(x=>x.id===id ? {...x,difficulty:value,problem_dna:dna} : x));
  ...
  return <main className="difficulty-page">
      ...
      {testResults.length>0 && <section className="test-section">
        ...
        <div className="test-grid">{testResults.map((x)=><article key={x.id} className={`test-card ${x.result?.ok ? (x.before !== String(x.result.difficulty) ? "changed" : "same") : "failed"}`}>
          ...
          <div className="difficulty-compare">
            <div><small>기존 난이도</small><strong>{x.before}</strong></div><span>→</span><div><small>AI 제안</small><strong className={x.result?.ok ? "proposal" : "fail-text"}>{x.result?.ok ? x.result.difficulty : "실패"}</strong></div>
          </div>
          {x.result?.ok ? <>
            ...
            <div className="review-actions">
              <button className="accept" disabled={norm(x.difficulty)===String(x.result.difficulty)} onClick={()=>void changeDifficulty(x.id,String(x.result?.difficulty ?? x.before))}>{norm(x.difficulty)===String(x.result.difficulty) ? "현재값과 동일" : `AI 제안 ${x.result.difficulty} 적용`}</button>
              <label><span>직접 수정</span><select value={norm(x.difficulty)} onChange={e=>void changeDifficulty(x.id,e.target.value)}>{D.map(d=><option key={d} value={d}>{d}단계</option>)}</select></label>
            </div>
