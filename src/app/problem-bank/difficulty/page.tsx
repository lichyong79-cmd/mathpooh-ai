...
import { DIFFICULTY_SCALE, DIFFICULTY_SCALE_VERSION, difficultyLabel, normalizeProblemDifficulty } from "@/lib/difficulty-scale";
...
export default function DifficultyManagementPage() {
  ...
  async function runTest20() {
    ...
    const targets = [...unreviewed].sort(() => Math.random() - 0.5).slice(0,20);
    if (!targets.length) { setError("아직 검수하지 않은 문항이 없습니다."); return; }
    if (!window.confirm(`새 SOS 8단계 기준으로 ${targets.length}문항을 AI가 판정합니다. 테스트만으로 DB는 변경되지 않습니다. 진행할까요?`)) return;
    setRunning(true); setMessage(""); setError(""); setTestResults([]);
    setProgress({ mode:"test", done:0, total:targets.length, ok:0, fail:0 });
    const rows: TestRow[] = []; let ok=0, fail=0;
    try {
      for (let i=0;i<targets.length;i++) {
        const target=targets[i];
        const res=await fetch("/api/problem-bank/regrade-difficulty",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({problemId:target.id,dryRun:true})});
        const data=await res.json().catch(()=>({})) as RegradeResult; const result={...data,ok:res.ok&&data?.success===true};
        result.ok?ok++:fail++; rows.push({...target,before:norm(target.difficulty,target.problem_dna),result}); setTestResults([...rows]);
        setProgress({mode:"test",done:i+1,total:targets.length,ok,fail});
      }
      setMessage(`8단계 20문항 테스트 완료 · 성공 ${ok} · 실패 ${fail} · DB는 변경하지 않았습니다.`);
  ...
  <main className="difficulty-page">
    <div className="difficulty-wrap">
      ...
      {testResults.length>0 && <section className="test-section">
        <div className="test-section-head"><div><b>8단계 20문항 테스트 결과</b><span>문제를 직접 보고 AI 제안을 검수하세요. 테스트만으로는 DB가 변경되지 않습니다.</span></div><strong>{testResults.filter(x=>x.result?.ok).length}/{testResults.length} 성공</strong></div>
        <div className="test-grid">{testResults.map((x)=><article key={x.id} className={`test-card ${x.result?.ok ? (x.before !== String(x.result.difficulty) ? "changed" : "same") : "failed"}`}>
          ...
          {x.result?.ok ? <>
            ...
            <div className="review-actions">
              <button className="accept" disabled={savingId===x.id || norm(x.difficulty,x.problem_dna)===String(x.result.difficulty)} onClick={()=>void changeDifficulty(x.id,String(x.result?.difficulty ?? x.before))}>{savingId===x.id ? "저장 중..." : norm(x.difficulty,x.problem_dna)===String(x.result.difficulty) ? "현재값과 동일" : `AI 제안 ${difficultyLabel(x.result.difficulty)} 적용`}</button>
