...
export async function POST(request: Request) {
  ...
  const created = await supabase.auth.admin.createUser({ email: `${phone}@student.matspu.local`, password: phone.slice(-4), email_confirm: true, user_metadata: { role: "student", student_id: student.id, name: student.name } });
