...
export async function GET(request:Request){
 ...
 try{
  ...
  if(detailId){
   ...
   // SOS252.1: async map 콜백 안에서 Supabase maybeSingle()의 data narrowing이 풀리는 TS 빌드 오류 방지
   const sessionData:any=s.data;
   const [student,logs,resets]=await Promise.all([
    ctx.supabase.from("students").select("id,name,school,grade,status").eq("id",sessionData.student_id).maybeSingle(),
   ...
   const logRows=logs.data??[];const rawItems=(sessionData.sos_training_items??[]).sort((a:any,b:any)=>Number(a.item_order)-Number(b.item_order));
   const items=await Promise.all(rawItems.map(async(item:any)=>{const problem=Array.isArray(item.problem_bank_questions)?item.problem_bank_questions[0]:item.problem_bank_questions??null;const generated=item.generated_problem??null;const itemLogs=logRows.filter((l:any)=>String(l.item_id)===String(item.id));return {id:item.id,order:Number(item.item_order??0),studentAnswer:String(item.student_answer??""),isCorrect:item.is_correct,responseSeconds:item.response_seconds==null?null:Number(item.response_seconds),answeredAt:item.answered_at,revealedAt:item.revealed_at,answerLockedAt:item.answer_locked_at,photoSubmittedAt:item.photo_submitted_at,photoSubmitSeconds:item.photo_submit_seconds==null?null:Number(item.photo_submit_seconds),screenExitCount:Number(item.screen_exit_count??0),reviewAnswer:String(item.review_answer??""),reviewIsCorrect:item.review_is_correct,reviewResponseSeconds:item.review_response_seconds==null?null:Number(item.review_response_seconds),reviewAnsweredAt:item.review_answered_at??null,reviewAttemptCount:itemLogs.filter((l:any)=>["REVIEW_ITEM_RETRY_WRONG","REVIEW_ITEM_CORRECTED"].includes(String(l.event_type))).length,reviewAttempts:itemLogs.filter((l:any)=>["REVIEW_ITEM_RETRY_WRONG","REVIEW_ITEM_CORRECTED"].includes(String(l.event_type))).map((l:any,index:number)=>({attemptNo:Number(l.detail?.attemptNo??index+1),isCorrect:String(l.event_type)==="REVIEW_ITEM_CORRECTED"||l.detail?.isCorrect===true,answer:String(l.detail?.answer??""),responseSeconds:Number(l.detail?.responseSeconds??0)||0,occurredAt:l.occurred_at})).sort((a:any,b:any)=>a.attemptNo-b.attemptNo),reviewHints:itemLogs.filter((l:any)=>String(l.event_type)==="REVIEW_HINT_USED").map((l:any)=>({level:Number(l.detail?.level??0),hint:String(l.detail?.hint??""),occurredAt:l.occurred_at})),reviewExplained:itemLogs.some((l:any)=>String(l.event_type)==="REVIEW_ITEM_EXPLAINED"),solutionPhotoUrl:await signedUrl(ctx.supabase,"sos-solution-photos",item.solution_photo_path),generated:Boolean(generated),problem:{id:problem?.id??item.problem_id,code:String(problem?.problem_code??(generated?"AI-GENERATED":"")),title:String(problem?.title??generated?.topic??"AI 유사문항"),subject:String(problem?.subject??generated?.subject??""),unit:String(problem?.unit??generated?.subunit??""),topic:String(problem?.topic??generated?.topic??""),difficulty:String(problem?.difficulty??generated?.difficulty??""),difficultyMeter:problem?.difficulty_meter==null?(generated?.meter??null):Number(problem.difficulty_meter),correctAnswer:String(problem?.answer??generated?.answer??""),generatedText:String(generated?.question??""),generatedSolution:String(generated?.solution??""),imageUrl:problem?await signedUrl(ctx.supabase,"question-images",problem?.question_image_path):generated?generatedQuestionImageDataUrl(generated?.question,{topic:generated?.topic,kind:generated?.generationKind??sessionData.cycle_kind}):""}};}));
