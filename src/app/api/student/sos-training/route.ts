  ...
  if (studentResult.error || !studentResult.data)
    return {
      error: NextResponse.json(
        {
          message:
            studentResult.error?.message || "학생 정보를 찾을 수 없습니다.",
        },
        { status: 404 },
      ),
    };
...
async function signedQuestionImage(
  supabase: any,
...
async function currentTargetMeter(
  supabase: any,
  studentId: string,
  session: any,
) {
  ...
  if (!key)
    return clampMeter(
      session?.baseline_meter ??
        session?.target_snapshot?.studentDifficultyMeter ??
        3,
    );
  ...
  return clampMeter(
    result.data?.difficulty_meter,
    session?.baseline_meter ?? 3,
  );
}

async function setTargetMeter(
  supabase: any,
  studentId: string,
  session: any,
  meter: number,
  ...
  const update = await supabase
    .from("sos_student_subunit_meters")
    .update({
      difficulty_meter: clampMeter(meter),
...
export async function GET(request: Request) {
  ...
  if (params.get("mode") === "status") {
    ...
    return NextResponse.json(
      {
        success: true,
        hasWaiting: rows.some((j: any) =>
          ["QUEUED", "GENERATING"].includes(String(j.status)),
        ),
        hasFailed: rows.some((j: any) => String(j.status) === "FAILED"),
        latestUpdatedAt: rows[0]?.updated_at ?? null,
      },
      { headers: { "Cache-Control": "no-store" } },
    );
  }
  const focusSessionId = String(params.get("sessionId") ?? "");
  ...
  const rootOf = (session: any) => {
    ...
    while (cur?.parent_session_id && !seen.has(String(cur.id))) {
      seen.add(String(cur.id));
      const parent = sessionMap.get(String(cur.parent_session_id));
      if (!parent) break;
      cur = parent;
    }
    return cur ?? session;
  ...
  const sessions = await Promise.all(
    rawSessions.map(async (session: any) => {
        ...
        (session.sos_training_items ?? [])
          ...
          .map(async (item: any) => {
            ...
            return {
              ...
              reviewAttemptCount: (
                reviewLogMap.get(String(item.id)) ?? []
              ).filter((l: any) =>
                ["REVIEW_ITEM_RETRY_WRONG", "REVIEW_ITEM_CORRECTED"].includes(
                  String(l.event_type),
                ),
              ).length,
              reviewHintLevel: Math.max(
                0,
                ...(reviewLogMap.get(String(item.id)) ?? [])
                  .filter(
                    (l: any) => String(l.event_type) === "REVIEW_HINT_USED",
                  )
                  .map((l: any) => Number(l.detail?.level ?? 0)),
              ),
              reviewLastHint:
                [...(reviewLogMap.get(String(item.id)) ?? [])]
                  .reverse()
                  .find((l: any) => String(l.event_type) === "REVIEW_HINT_USED")
                  ?.detail?.hint ?? "",
              reviewExplained: (reviewLogMap.get(String(item.id)) ?? []).some(
                (l: any) => String(l.event_type) === "REVIEW_ITEM_EXPLAINED",
              ),
              reviewCompleted:
                item.review_is_correct === true ||
                (reviewLogMap.get(String(item.id)) ?? []).some(
                  (l: any) => String(l.event_type) === "REVIEW_ITEM_EXPLAINED",
                ),
              ...
              problem: {
                ...
                subject:
                  bank?.subject ??
                  generated?.subject ??
                  session.target_snapshot?.subject ??
                  "",
                unit:
                  bank?.unit ??
                  generated?.subunit ??
                  session.target_snapshot?.subunit ??
                  "",
                ...
                imageUrl:
                  ...
                  (["ASSIGNED", "IN_PROGRESS", "RETRAIN"].includes(
                    ...
                    ? await signedQuestionImage(
                        supabase,
                        bank.question_image_path,
                      )
                    : "",
                renderBlocks: Array.isArray(generated?.renderBlocks)
                  ? generated.renderBlocks
                  : [],
                ...
                generationKind:
                  generated?.generationKind ?? session.cycle_kind ?? "",
                // SOS281: 예전에는 cycle_kind가 HOMEWORK면 상태와 무관하게 풀이를 함께 내려보냈다.
                // 3제 굳히기는 시간제한이 없어서, 학생이 풀기 전에 개발자도구로 정답을 볼 수 있었다.
                // 이제 끝난 세션에서만 내려보낸다.
                generatedSolution: ["PASSED", "COMPLETED"].includes(
                  String(session.status),
                )
                  ? (generated?.solution ?? "")
                  : undefined,
                correctAnswer: ["COMPLETED", "PASSED", "RETRAIN"].includes(
                  String(session.status),
                )
                  ? String(answer)
                  : undefined,
      ...
      const learningCycle = cycleFromSnapshot(
        root?.target_snapshot ?? session?.target_snapshot,
      );
      return {
        ...session,
        items,
        learningCycle,
        cycleRootId: String(root?.id ?? session.id),
        sourceExam: {
          id:
            root?.target_snapshot?.sourceExamId ??
            session?.target_snapshot?.sourceExamId ??
            null,
          title:
            root?.target_snapshot?.sourceExamTitle ??
            session?.target_snapshot?.sourceExamTitle ??
            "기준 시험",
          attemptId:
            root?.target_snapshot?.sourceAttemptId ??
            session?.target_snapshot?.sourceAttemptId ??
            null,
        },
      };
    }),
  ...
  const meters = await supabase
    .from("sos_student_subunit_meters")
    .select(
      "subject,major_unit,subunit,subunit_key,difficulty_meter,sample_count,updated_at",
    )
    .eq("student_id", student.id)
  ...
  const jobsResult = await supabase
    .from("sos_ai_generation_jobs")
    .select(
      "id,source_training_session_id,generation_kind,requested_count,status,attempt_count,last_error,result_session_id,requested_at,started_at,completed_at,updated_at,pipeline_version,stage,stage_index,stage_total,stage_message,stage_updated_at",
    )
    .eq("student_id", student.id)
    .order("requested_at", { ascending: false });
  return NextResponse.json(
...
export async function POST(request: Request) {
  ...
  const sessionResult = await supabase
    .from("sos_training_sessions")
    .select(
      "id,student_id,phase,status,total_count,correct_count,decision,round_no,target_snapshot,weakness_snapshot,baseline_meter,goal_meter,cycle_kind,parent_session_id",
    )
    .eq("id", sessionId)
    .eq("student_id", student.id)
    .single();
  ...
  if (sessionResult.error || !sessionResult.data)
    return NextResponse.json(
      {
        message:
          sessionResult.error?.message || "진단·훈련을 찾을 수 없습니다.",
      },
      { status: 404 },
    );
  ...
  if (action === "recover_diagnosis") {
    if (
      String(session.phase) !== "DIAGNOSIS" ||
      !["COMPLETED", "PASSED"].includes(String(session.status))
    )
      return NextResponse.json(
        { message: "완료된 진단만 이어서 준비할 수 있습니다." },
        { status: 409 },
      );
    try {
      ...
      return NextResponse.json(
        {
          message:
            error instanceof Error ? error.message : "기존 진단 이어가기 실패",
        },
        { status: 500 },
      );
    }
  ...
  if (action === "ensure_next") {
    ...
    try {
      const all = await supabase
        .from("sos_training_sessions")
        .select(
          "id,phase,status,round_no,cycle_kind,parent_session_id,total_count,correct_count,decision,target_snapshot,baseline_meter,goal_meter,created_at",
        )
      ...
      const secondDiagnosis =
        cycleSessions.find(
          (x: any) =>
            String(x.phase) === "DIAGNOSIS" && Number(x.round_no) === 2,
        ) ?? null;
      ...
      if (homework && (isOpen(homework) || isDone(homework))) {
        return NextResponse.json({
          success: true,
          next: homework,
          nextStep: "HOMEWORK_ASSIGNED",
          existing: true,
        });
      }
      if (
        secondTraining &&
        (isOpen(secondTraining) || isDone(secondTraining))
      ) {
        return NextResponse.json({
          success: true,
          next: secondTraining,
          nextStep: "SECOND_TRAINING_ASSIGNED",
          existing: true,
        });
      }
      ...
      if (phase === "DIAGNOSIS") {
        if (firstTraining && (isOpen(firstTraining) || isDone(firstTraining))) {
          ...
          if (isDone(firstTraining)) {
            const meter = await currentTargetMeter(
              supabase,
              String(student.id),
              firstTraining,
            );
            const goal = clampMeter(
              firstTraining.goal_meter,
              Number(firstTraining.baseline_meter) || meter,
            );
            const scoreRate =
              Number(firstTraining.total_count) > 0
                ? Number(firstTraining.correct_count ?? 0) /
                  Number(firstTraining.total_count)
                : 0;
            const passed =
              String(firstTraining.decision) === "FIRST_TRAINING_PASSED" ||
              scoreRate >= 0.9 ||
              meter >= goal;
            const queued = await enqueueAiGeneration({
              supabase,
              studentId: String(student.id),
              sourceTrainingSessionId: String(firstTraining.id),
              count: passed ? 3 : 10,
            ...
            return NextResponse.json({
              ...
              queued: true,
              job: queued.job,
              nextStep: "AI_GENERATION_QUEUED",
              generationKind: passed ? "HOMEWORK" : "SECOND_TRAINING",
          ...
          return NextResponse.json({
            success: true,
            next: firstTraining,
            nextStep: "FIRST_TRAINING_ASSIGNED",
            existing: true,
          });
        }
        ...
        if (
          round === 1 &&
          String(session.decision) === "PERFECT_DIAGNOSIS_AUTO_NEXT"
        ) {
          if (
            secondDiagnosis &&
            (isOpen(secondDiagnosis) || isDone(secondDiagnosis))
          ) {
            return NextResponse.json({
              success: true,
              next: secondDiagnosis,
              nextStep: "SECOND_DIAGNOSIS_ASSIGNED",
              existing: true,
            });
          }
          const next = await createAutomaticSecondDiagnosis({
            ...
            parentDiagnosis: {
              ...session,
              id: sessionId,
              target_snapshot: session.target_snapshot,
            },
          });
          return NextResponse.json({
            success: true,
            next,
            nextStep: next?.nextStep ?? "SECOND_DIAGNOSIS_ASSIGNED",
          });
        }
        ...
        if (isDone(session)) {
          ...
          return NextResponse.json({
            ...
            next: ai,
            nextStep: ai?.created
              ? "FIRST_TRAINING_ASSIGNED"
              : (ai?.nextStep ?? "DIAGNOSIS_COMPLETE_NO_WEAKNESS"),
          });
        }

        return NextResponse.json(
          { message: "아직 다음 진단·훈련을 만들 단계가 아닙니다." },
          { status: 409 },
        );
      }
      ...
      if (
        ...
        const goal = clampMeter(
          session.goal_meter,
          Number(session.baseline_meter) || meter,
        );
        const scoreRate =
          Number(session.total_count) > 0
            ...
            : 0;
        const passed =
          String(session.decision) === "FIRST_TRAINING_PASSED" ||
          scoreRate >= 0.9 ||
          meter >= goal;
        const queued = await enqueueAiGeneration({
          supabase,
      ...
      return NextResponse.json(
        {
          success: false,
          message:
            error instanceof Error ? error.message : "다음 학습 자동 준비 실패",
          retryable: true,
        },
        { status: 500 },
      );
  ...
  if (action === "reveal") {
    if (
      ...
      return NextResponse.json(
        { message: "진행 중인 진단에서만 문항을 공개할 수 있습니다." },
        { status: 409 },
      );
    const itemId = String(body.itemId ?? "");
    const item = await supabase
      .from("sos_training_items")
      .select("id,revealed_at,answer_locked_at")
      .eq("id", itemId)
      .eq("session_id", sessionId)
      .single();
    if (item.error || !item.data)
      return NextResponse.json(
        { message: item.error?.message || "문항을 찾을 수 없습니다." },
        { status: 404 },
      );
    if (item.data.answer_locked_at)
      return NextResponse.json(
        { message: "이미 답안을 확정한 문항입니다." },
        { status: 409 },
      );
  ...
  if (action === "lock_answer") {
    if (
      ...
      return NextResponse.json(
        { message: "진행 중인 진단에서만 답안을 확정할 수 있습니다." },
        { status: 409 },
      );
    const itemId = String(body.itemId ?? "");
    ...
    const item = await supabase
      .from("sos_training_items")
      .select("id,revealed_at,answer_locked_at,student_answer")
      .eq("id", itemId)
      .eq("session_id", sessionId)
      .single();
    if (item.error || !item.data)
    ...
    if (item.data.answer_locked_at)
      return NextResponse.json(
        { message: "이미 확정한 답안입니다." },
        { status: 409 },
      );
    if (!item.data.revealed_at)
      return NextResponse.json(
        { message: "아직 공개되지 않은 문항입니다." },
        { status: 409 },
      );
    ...
    const seconds = Math.max(
      1,
      Math.round(
        (lockedAt.getTime() - new Date(item.data.revealed_at).getTime()) / 1000,
      ),
    );
    const update = await supabase
      .from("sos_training_items")
      .update({
        ...
        answer_locked_at: lockedAt.toISOString(),
        answered_at: lockedAt.toISOString(),
      })
      .eq("id", itemId);
    if (update.error)
      return NextResponse.json(
    ...
    await supabase.from("sos_training_activity_logs").insert({
      ...
      item_id: itemId,
      student_id: student.id,
      event_type: "ANSWER_LOCKED",
      detail: { responseSeconds: seconds },
    });
    return NextResponse.json({
      success: true,
      responseSeconds: seconds,
      answerLockedAt: lockedAt.toISOString(),
    });
  }
  ...
  if (action === "activity") {
    ...
    if (!["SCREEN_EXIT", "SCREEN_RETURN"].includes(eventType))
      return NextResponse.json(
        { message: "지원하지 않는 로그입니다." },
        { status: 400 },
      );
    const detail =
      body.detail && typeof body.detail === "object" ? body.detail : {};
    const log = await supabase.from("sos_training_activity_logs").insert({
      session_id: sessionId,
      item_id: itemId,
      student_id: student.id,
    ...
    if (eventType === "SCREEN_EXIT" && itemId) {
      ...
      if (!row.error && row.data)
        await supabase
          .from("sos_training_items")
          .update({
            screen_exit_count: Number(row.data.screen_exit_count ?? 0) + 1,
          })
          .eq("id", itemId);
    }
  ...
  if (action === "item_solution") {
    if (!["COMPLETED", "PASSED", "RETRAIN"].includes(String(session.status)))
      return NextResponse.json(
        { message: "학습이 끝난 뒤에 해설을 볼 수 있습니다." },
        { status: 403 },
      );
    const itemId = String(body.itemId ?? "");
    if (!itemId)
    ...
    if (found.error || !found.data)
      return NextResponse.json(
        { message: "문항을 찾지 못했습니다." },
        { status: 404 },
      );

    const generated: any = found.data.generated_problem ?? {};
    const bank: any = Array.isArray(found.data.problem_bank_questions)
      ? found.data.problem_bank_questions[0]
      : found.data.problem_bank_questions;
    const correctAnswer = String(
      bank?.answer ?? generated?.answer ?? "",
    ).trim();
    ...
    return NextResponse.json({
      success: true,
      itemOrder: Number(found.data.item_order ?? 0),
      correctAnswer,
      solutionText,
      solutionImageUrl,
  ...
  if (action === "open_training_item") {
    ...
    if (found.data.revealed_at)
      return NextResponse.json({
        success: true,
        revealedAt: found.data.revealed_at,
      });
  ...
  if (action === "save_training_item") {
    if (
      ...
      return NextResponse.json(
        { message: "진행 중인 훈련에서만 저장할 수 있습니다." },
        { status: 409 },
      );
    const itemId = String(body.itemId ?? "");
    const answer = String(body.answer ?? "").trim();
    const clientSeconds = Math.max(
      1,
      Math.round(Number(body.responseSeconds ?? 0) || 1),
    );
    if (!itemId || !answer)
      return NextResponse.json(
        { message: "문항과 답을 확인해 주세요." },
        { status: 400 },
      );
    ...
    const current = await supabase
      .from("sos_training_items")
      .select("id,revealed_at,response_seconds")
      .eq("id", itemId)
      .eq("session_id", sessionId)
      .maybeSingle();
    const revealedAt = current.data?.revealed_at
      ? Date.parse(String(current.data.revealed_at))
      : NaN;
    ...
    await supabase.from("sos_training_activity_logs").insert({
      ...
      item_id: itemId,
      student_id: student.id,
      event_type: "TRAINING_ITEM_DONE",
      detail: {
        question: Number(body.question ?? 0),
        responseSeconds,
        clientSeconds,
      },
    });
  ...
  if (action === "save_review_item") {
    if (
      !["DIAGNOSIS", "TRAINING"].includes(String(session.phase)) ||
      String(session.status) !== "RETRAIN"
    )
      return NextResponse.json(
        { message: "오답 교정 진행 상태가 아닙니다." },
        { status: 409 },
      );
    const itemId = String(body.itemId ?? "");
    const answer = String(body.answer ?? "").trim();
    ...
    if (!itemId || !answer)
      return NextResponse.json(
        { message: "오답 문항과 답을 확인해 주세요." },
        { status: 400 },
      );
    const itemResult = await supabase
      .from("sos_training_items")
      .select(
        "id,generated_problem,problem_bank_questions(id,title,subject,unit,topic,difficulty,answer)",
      )
      .eq("id", itemId)
      .eq("session_id", sessionId)
      .single();
    if (itemResult.error || !itemResult.data)
      return NextResponse.json(
        {
          message: itemResult.error?.message || "오답 문항을 찾을 수 없습니다.",
        },
        { status: 404 },
      );
    const row: any = itemResult.data;
    const bank: any = Array.isArray(row.problem_bank_questions)
      ? row.problem_bank_questions[0]
      : row.problem_bank_questions;
    const generated: any = row.generated_problem ?? null;
    const correctAnswer = String(
      bank?.answer ?? generated?.answer ?? "",
    ).trim();
    const isCorrect = answerMatches(answer, correctAnswer);
    const priorLogs = await supabase
      .from("sos_training_activity_logs")
      .select("event_type")
      .eq("session_id", sessionId)
      .eq("item_id", itemId)
    ...
    const update = await supabase
      ...
      .update({
        ...
        review_response_seconds: responseSeconds,
        review_answered_at: new Date().toISOString(),
      })
      .eq("id", itemId)
      .eq("session_id", sessionId);
    if (update.error)
    ...
    if (attemptNo <= 2) {
      const hint = await generateReviewHint({
        problem: {
          title: bank?.title ?? generated?.topic,
          subject: bank?.subject ?? generated?.subject,
          unit: bank?.unit ?? generated?.subunit,
          topic: bank?.topic ?? generated?.topic,
          difficulty: bank?.difficulty ?? generated?.difficulty,
          generatedText: generated?.question,
        },
        studentAnswer: answer,
        attempt: attemptNo,
        weakness: session.weakness_snapshot ?? session.target_snapshot,
      });
      await supabase.from("sos_training_activity_logs").insert({
        session_id: sessionId,
        item_id: itemId,
        student_id: student.id,
        event_type: "REVIEW_HINT_USED",
        detail: {
          question: Number(body.question ?? 0),
          level: attemptNo,
          hint,
  ...
  if (action === "complete_review_explanation") {
    ...
    await supabase.from("sos_training_activity_logs").insert({
      ...
      item_id: itemId,
      student_id: student.id,
      event_type: "REVIEW_ITEM_EXPLAINED",
      detail: { question: Number(body.question ?? 0) },
    });
  ...
  if (action === "submit_review") {
    ...
    const reviewSeconds = (body.responseSeconds ?? {}) as Record<
      string,
      unknown
    >;
    const itemsResult = await supabase
      .from("sos_training_items")
      .select(
        "id,problem_id,is_correct,problem_meter_before,generated_problem,problem_bank_questions(answer)",
      )
      .eq("session_id", sessionId)
      .eq("is_correct", false)
      .order("item_order");
    ...
    if (String(session.cycle_kind) === "HOMEWORK") {
      const explanationLogs = await supabase
        .from("sos_training_activity_logs")
        .select("item_id,event_type")
        .eq("session_id", sessionId)
        .eq("event_type", "REVIEW_ITEM_EXPLAINED");
      const explained = new Set(
        (explanationLogs.data ?? []).map((l: any) => String(l.item_id)),
      );
      const reviewRows = await supabase
        .from("sos_training_items")
        .select("id,review_is_correct")
        .eq("session_id", sessionId)
        .eq("is_correct", false);
      if (reviewRows.error)
        return NextResponse.json(
          { message: reviewRows.error.message },
          { status: 400 },
        );
      const unfinished = (reviewRows.data ?? []).filter(
        (r: any) =>
          r.review_is_correct !== true && !explained.has(String(r.id)),
      );
      if (unfinished.length)
        return NextResponse.json(
          {
            message: `아직 교정하지 않은 숙제 오답이 ${unfinished.length}문항 있습니다.`,
          },
          { status: 400 },
        );
      const current = await currentTargetMeter(
        supabase,
        String(student.id),
        session,
      );
      const update = await supabase
        ...
        .update({
          status: "PASSED",
          decision: "HOMEWORK_DONE",
          review_meter: current,
          updated_at: new Date().toISOString(),
        })
        .eq("id", sessionId);
      if (update.error)
      ...
      await supabase.from("sos_training_activity_logs").insert({
        ...
        detail: {
          phase: "HOMEWORK",
          corrected: (reviewRows.data ?? []).filter(
            (r: any) => r.review_is_correct === true,
          ).length,
          total: (reviewRows.data ?? []).length,
        },
      });
      return NextResponse.json({
        success: true,
    ...
    if (String(session.phase) === "DIAGNOSIS") {
      ...
      const reviewRows = await supabase
        .from("sos_training_items")
        .select("id,review_answer,review_is_correct")
        .eq("session_id", sessionId)
        .eq("is_correct", false);
      if (reviewRows.error)
        return NextResponse.json(
      ...
      if (unfinished.length)
        return NextResponse.json(
          {
            message: `아직 교정하지 않은 진단 오답이 ${unfinished.length}문항 있습니다.`,
          },
          { status: 400 },
        );
      const update = await supabase
      ...
      let ai: any = null;
      try {
        ai = await analyzeDiagnosisAndCreateFirstTraining({
          supabase,
          studentId: String(student.id),
          diagnosisSessionId: sessionId,
        });
        ...
        ai = {
          error: error instanceof Error ? error.message : "AI 취약점 분석 실패",
        };
      }
      return NextResponse.json({
        success: true,
        ...
        nextStep: ai?.created
          ? "FIRST_TRAINING_ASSIGNED"
          : (ai?.nextStep ?? "AI_REVIEW_REQUIRED"),
      });
    }
    ...
    if (!wrongItems.length) {
      ...
      if (Number(session.round_no) === 1) {
        try {
          ...
          console.error("[SOS264_QUEUE_AFTER_REVIEW]", error);
        }
      }
      return NextResponse.json({
        success: true,
        ...
        next,
        aiJob,
        nextStep: aiJob ? "AI_GENERATION_QUEUED" : undefined,
      });
    }
    ...
    for (const item of wrongItems as any[]) {
      ...
      if (!answer)
        return NextResponse.json(
          { message: "오답 문항을 모두 다시 풀어 주세요." },
          { status: 400 },
        );
      const seconds = Math.max(
        1,
        Math.round(Number(reviewSeconds[id] ?? 0) || 1),
      );
      const reviewBank: any = Array.isArray(item.problem_bank_questions)
        ? item.problem_bank_questions[0]
        : item.problem_bank_questions;
      const correctAnswer = String(
        reviewBank?.answer ?? item.generated_problem?.answer ?? "",
      );
      ...
      const b = reviewBonus({
        ...
        responseSeconds: seconds,
        problemMeter: Number(
          item.problem_meter_before ?? item.generated_problem?.meter ?? 3,
        ),
      });
      bonusTotal += b;
      const update = await supabase
        ...
        .update({
          review_answer: answer,
          review_is_correct: ok,
          review_response_seconds: seconds,
          review_answered_at: new Date().toISOString(),
        })
        .eq("id", id);
      if (update.error)
    ...
    if (sessionUpdate.error)
      return NextResponse.json(
        { message: sessionUpdate.error.message },
        { status: 400 },
      );
    await supabase.from("sos_training_activity_logs").insert({
      session_id: sessionId,
  ...
  if (action === "submit") {
    ...
    if (items.length !== Number(session.total_count))
      return NextResponse.json(
        {
          message: `문항 구성이 올바르지 않습니다. ${items.length}/${session.total_count}`,
        },
        { status: 409 },
      );
    const fullById = new Map<string, any>(
      items.map((x: any) => [String(x.id), x] as [string, any]),
    );
    const missing = items.filter(
      (item: any) =>
        !String(
          answers[String(item.id)] ??
            fullById.get(String(item.id))?.student_answer ??
            "",
        ).trim(),
    );
    if (missing.length)
      return NextResponse.json(
        {
          message: `미응답 ${missing.length}문항이 있습니다. 모두 답한 뒤 제출해 주세요.`,
        },
        { status: 400 },
      );
    if (String(session.phase) === "DIAGNOSIS") {
      const unlocked = items.filter(
        (item: any) => !fullById.get(String(item.id))?.answer_locked_at,
      );
      if (unlocked.length)
        return NextResponse.json(
          { message: "모든 진단 문항의 답안을 먼저 확정해 주세요." },
          { status: 400 },
        );
      const noPhoto = items.filter(
        (item: any) => !fullById.get(String(item.id))?.solution_photo_path,
      );
      if (noPhoto.length)
        return NextResponse.json(
          { message: `풀이사진 미제출 ${noPhoto.length}문항이 있습니다.` },
          { status: 400 },
        );
    }
    ...
    if (String(session.cycle_kind) === "HOMEWORK") {
      ...
      for (const item of items as any[]) {
        ...
        const bank: any = Array.isArray(item.problem_bank_questions)
          ? item.problem_bank_questions[0]
          : item.problem_bank_questions;
        const correctAnswer = String(
          bank?.answer ?? generated?.answer ?? "",
        ...
        const responseSeconds = Math.max(
          ...
          Math.round(
            Number(
              seconds[String(item.id)] ??
                fullById.get(String(item.id))?.response_seconds ??
                0,
            ) || 1,
          ),
        );
        ...
        const update = await supabase
          ...
          .update({
            ...
            response_seconds: responseSeconds,
            answered_at: gradedAt,
            student_meter_before: null,
            student_meter_after: null,
            problem_meter_before: null,
            problem_meter_after: null,
          })
      ...
      const update = await supabase
        ...
        .update({
          ...
          decision:
            wrong > 0
              ? "HOMEWORK_REVIEW_REQUIRED"
              : "HOMEWORK_RESULT_REVIEW_READY",
          training_meter: current,
          updated_at: gradedAt,
        })
        .eq("id", sessionId);
      if (update.error)
    ...
    const problemIds = [
      ...new Set(
        items.map((x: any) => String(x.problem_id ?? "")).filter(Boolean),
      ),
    ];
    ...
    if (eventRows.error)
      return NextResponse.json(
        { message: eventRows.error.message },
        { status: 400 },
      );
    if (priorProblemRows.error)
      return NextResponse.json(
        { message: priorProblemRows.error.message },
        { status: 400 },
      );
    const duplicateItems = new Set<string>(
      (eventRows.data ?? []).map((x: any) => String(x.training_item_id)),
    );
    const seenProblemIds = new Set<string>(
      (priorProblemRows.data ?? []).map((x: any) => String(x.problem_id)),
    );
    const meterCache = new Map<
      string,
      { id: string; meter: number; samples: number }
    >();
    ...
    if (phase === "DIAGNOSIS") {
      ...
      if (wrong === 0) {
        ...
        try {
          ...
          ai = {
            error: error instanceof Error ? error.message : "진단 AI 분석 실패",
            nextStep: "AI_REVIEW_REQUIRED",
          };
        }
        const nextStep =
          ai?.created || ai?.existing
            ? "FIRST_TRAINING_ASSIGNED"
            : (ai?.nextStep ?? "AI_REVIEW_REQUIRED");
        await supabase.from("sos_training_activity_logs").insert({
          session_id: sessionId,
        ...
        return NextResponse.json({
          ...
          message:
            nextStep === "SECOND_DIAGNOSIS_ASSIGNED"
              ? "진단 완료 · 2차 진단을 자동 준비했습니다."
              : nextStep === "FIRST_TRAINING_ASSIGNED"
                ? "진단 완료 · 정오답과 풀이시간을 분석해 1차 맞춤훈련을 준비했습니다."
                : nextStep === "DIAGNOSIS_COMPLETE_NO_WEAKNESS"
                  ? "1·2차 진단에서 오답과 시간취약이 없어 이번 SOS를 완료했습니다."
                  : "진단 완료 · 다음 학습 준비 상태를 확인합니다.",
        });
      }
      ...
      const update = await supabase
        ...
        .update({
          status: "RETRAIN",
          correct_count: correct,
          decision: "DIAGNOSIS_REVIEW_REQUIRED",
          updated_at: new Date().toISOString(),
        })
        .eq("id", sessionId);
      if (update.error)
