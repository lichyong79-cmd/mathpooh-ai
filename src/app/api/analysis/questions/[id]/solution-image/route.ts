...
export async function GET(_: NextRequest, context: { params: Promise<{ id: string }> }) {
  ...
  try {
    ...
    const question = await supabase.from("analysis_questions").select("ai_result").eq("id", id).single();
    const path = String(question.data?.ai_result?.official_solution_image_path ?? "").trim();
    if (question.error || !path) return NextResponse.json({ success: false, message: "문항별 공식 해설 이미지가 없습니다." }, { status: 404 });
