...
type Exam = { id: string; title: string; exam_code: string; exam_date: string; grade: string; subject: string; exam_range: string; question_count: number; time_limit: number; total_score: number; objective_count: number; short_answer_count: number; test_url: string; available: boolean; application_status: "none" | "requested" | "assigned"; attempt: Attempt | null };
...
export default function StudentHome() {
  ...
  const changeApplication = async (exam: Exam, action: "request" | "cancel-request") => {
    if (busy) return;
    if (action === "cancel-request" && !window.confirm("이 시험 신청을 취소할까요?")) return;
    setBusy(action === "request" ? "시험을 신청하고 있습니다..." : "시험 신청을 취소하고 있습니다...");
    const response = await fetch("/api/student/portal", { method: "POST", headers: { "content-type": "application/json" }, body: JSON.stringify({ action, examId: exam.id }) });
    const data = await response.json();
    setBusy("");
    if (!response.ok) return alert(data.message || "시험 신청을 처리하지 못했습니다.");
    await load();
  };
  ...
  return <main className="student-portal">{busy ? <div className="student-busy"><div><b>{busy}</b><span>잠시 기다려 주세요.</span></div></div> : null}<header className="student-hero"><div><small>MATSPU SOS</small><h1>{portal.student.name} 학생</h1><p>{portal.student.school} · {portal.student.grade}</p></div><div><button onClick={() => window.location.href = "/password"}>비밀번호 변경</button><button onClick={() => void signOut()}>로그아웃</button></div></header><section className="student-welcome"><div><span>이번 주 목표</span><h2>아래 점수부터 하나씩 확보합니다.</h2><p>시험을 신청하고 배정이 완료되면 온라인으로 응시할 수 있습니다.</p></div><b>{portal.exams.filter((exam) => exam.application_status === "assigned" && exam.attempt?.status !== "submitted").length}<small>배정 완료</small></b></section><section className="student-exam-list"><h2>실전모의고사 신청·응시</h2>{portal.exams.map((exam) => <article key={exam.id}><div className="exam-date"><b>{new Date(exam.exam_date).getDate()}</b><span>{new Date(exam.exam_date).toLocaleDateString("ko-KR", { month: "short" })}</span></div><div className="exam-info"><small>{exam.exam_code}</small><h3>{exam.title}</h3><p>{exam.subject} · {exam.question_count}문항 · {exam.time_limit}분</p></div><div className="exam-state">{exam.attempt?.status === "submitted" ? <><b className="complete">제출 완료</b><strong>{exam.attempt.score}점</strong></> : exam.application_status === "none" ? <><b>신청 가능</b><button onClick={() => void changeApplication(exam, "request")}>시험 신청</button></> : exam.application_status === "requested" ? <><b>배정 대기</b><button onClick={() => void changeApplication(exam, "cancel-request")}>신청 취소</button></> : <><b>{exam.attempt ? "응시 중" : "배정 완료"}</b><button disabled={!exam.available || !exam.test_url} onClick={() => void startExam(exam)}>{exam.attempt ? "이어서 풀기" : exam.available ? "시험 시작" : "응시시간 대기"}</button></>}</div></article>)}{portal.exams.length === 0 ? <div className="student-empty">현재 신청 가능한 시험이 없습니다.</div> : null}</section></main>;
}
