...
export default function PdfMapperPage(){
  ...
  return <main className="mapper-shell">
    ...
    <div className="mapper-grid">
      ...
      <section className="viewer-card"><div className="viewer-toolbar"><button disabled={page<=1} onClick={()=>setPage(p=>p-1)}>이전</button><b>{pageCount?`${page}/${pageCount} 페이지`:"PDF 로딩"}</b><button disabled={!pageCount||page>=pageCount} onClick={()=>setPage(p=>p+1)}>다음</button><span>{active}번 확인 중</span></div><div className="canvas-wrap">{loading&&<MATHPOOHLoader title="시험지를 불러오는 중입니다" detail="PDF와 문항 좌표를 준비하고 있습니다." kind="exam" audience="admin"/>}<canvas ref={canvasRef}/>{pdfDoc&&<div ref={overlayRef} className="overlay" onPointerDown={down} onPointerMove={move} onPointerUp={up}>{pageRegions.map(r=><button key={r.number} className={`region ${r.number===active?"active":""} ${r.verified?"verified":""}`} style={{left:`${r.x}%`,top:`${r.y}%`,width:`${r.w}%`,height:`${r.h}%`}} onPointerDown={e=>e.stopPropagation()} onClick={e=>{e.stopPropagation();setActive(r.number)}}>{r.number}</button>)}{draft&&<div className="region draft" style={{left:`${draft.x}%`,top:`${draft.y}%`,width:`${draft.w}%`,height:`${draft.h}%`}}/>}</div>}</div></section>
