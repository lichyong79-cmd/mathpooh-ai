    ...
    <article className="sos-diagnosis-question sos-training-question"><header><div><b>{item.order}번 오답</b><span>{done?"교정 완료":attempt===0?"정답을 보지 않고 다시 풀기":attempt<3?`힌트 ${attempt}단계 사용 중`:"정답·핵심풀이 확인 단계"}</span></div><div className="solve"><small>재풀이시간</small><strong>{fmt(elapsed)}</strong></div></header>
      ...
      <div className="sos-diagnosis-image">{item.generated
        ?<SosGeneratedQuestionPng
          ...
          topic={String(item.problem?.coreType??item.problem?.topic??"")}
        />
        :item.problem?.imageUrl
          ?<SosProblemImage src={item.problem.imageUrl} alt={`${item.order}번 오답 문제`} problemCode={String(item.problem?.code??"")} maskOriginalNumber={diagnosis||Number(session?.round_no??1)===1}/>
          :<p>문항 이미지가 없습니다.</p>}</div>
