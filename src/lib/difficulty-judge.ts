...
6=어4: 어려운 4점. 복합추론·경우분류·구조변환·상당한 계산/해석 부담. 단, 상위권에게까지 뚜렷한 변별 부담이 생기면 7을 검토한다.
7=준킬러: 상위권 학생에게도 진입 전략 선택, 여러 조건의 정교한 연결, 경우분류 또는 완주 부담 중 하나 이상이 뚜렷한 고난도. 낯선 기발한 발상이 필수조건은 아니다.
8=킬러: 최상위권 학생에게도 첫 발상 발견, 복합 추론, 분기 관리 또는 완주가 매우 어려운 최고난도권. '거의 못 푸는 문제'에만 예약하지 않는다.

[절대 규칙]
- 반드시 8개 등급을 하나의 연속된 절대 눈금으로 사용한다. 6을 사실상의 최고등급처럼 쓰거나 7/8을 예외값처럼 봉쇄하지 않는다.
- 먼저 이 문항이 2점급/3점급/4점급인지 독립적으로 결정하고, 4점급이면 쉬4→적4→어4→준킬러→킬러 다섯 구간 중 학생 체감 부담에 맞는 곳을 다시 고른다.
- '준킬러/킬러'라는 명칭 자체 때문에 등급을 낮추지 않는다. 실제 학생 관점의 진입·연결·완주 부담이 기준에 맞으면 7/8을 정상적으로 선택한다.
- 1 또는 2를 선택하려면 복수 조건의 의미 있는 연결, 전략 선택, 경우분류, 구조 변환 같은 4점급 부담이 실제로 거의 없다는 점을 확인한다. 단순히 풀이가 알려진 유형이거나 AI에게 쉬웠다는 이유로 1/2를 주지 않는다.
- 3(어3)은 3점의 상단이지 쉬4 이상의 복합 사고를 담는 완충지대가 아니다. 4점급 사고가 확인되면 4 이상으로 보낸다.
