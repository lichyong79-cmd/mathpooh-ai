...
export async function enqueueAiGeneration(args:{supabase:any;studentId:string;sourceTrainingSessionId:string;kind:AiGenerationKind;count:3|10}){
  ...
  if(existing.data){
    if(existing.data.status==="GENERATING"&&Date.now()-new Date(existing.data.started_at??0).getTime()>7*60000){
      const reset=await supabase.from("sos_ai_generation_jobs").update({status:"QUEUED",attempt_count:0,started_at:null,last_error:null,updated_at:new Date().toISOString()}).eq("id",existing.data.id).eq("status","GENERATING").lt("started_at",new Date(Date.now()-7*60000).toISOString()).select("*").maybeSingle();
      if(reset.error)throw reset.error;
      if(reset.data)return {job:reset.data,existing:true,retried:true};
    }
    // 실패 작업은 학생이 다시 이어갈 때 즉시 QUEUED로 되살린다.
    if(String(existing.data.status)==="FAILED"||(existing.data.status==="QUEUED"&&Number(existing.data.attempt_count)>=8)){
