import { SOURCE_STAR_POLICY, SOURCE_STAR_PROMPT, sourceStarSchema, type SourceStars } from "@/lib/source-star-difficulty";
...
export async function ensureOriginalSourceStars(supabase: any, problem: any, apiKey: string, model: string) {
  if (!problem.source_file_id) return problem.problem_dna;
  ...
  if (problem.problem_dna?.difficulty?.source_star_origin === "original_pdf" && problem.problem_dna?.difficulty?.source_star_policy === SOURCE_STAR_POLICY) return problem.problem_dna;
  const key = String(source.id);
  if (!pending.has(key)) pending.set(key, readPaper(supabase, source, apiKey, model).finally(() => pending.delete(key)));
...
async function readPaper(supabase: any, source: any, apiKey: string, model: string) {
  ...
  if (rows.length && rows.every((r: any) => r.problem_dna?.difficulty?.source_star_origin === "original_pdf" && r.problem_dna?.difficulty?.source_star_policy === SOURCE_STAR_POLICY)) return;
  ...
  for (const row of rows) {
    ...
    const fresh = await supabase.from("problem_bank_questions").select("problem_dna,updated_at").eq("id",row.id).single();
    if (fresh.error) throw fresh.error;
    const dna = fresh.data.problem_dna ?? {};
    const update = await supabase.from("problem_bank_questions").update({problem_dna:{...dna,difficulty:{...dna.difficulty,source_stars:evidence.get(Number(row.question_no)),source_star_policy:SOURCE_STAR_POLICY,source_star_origin:"original_pdf",source_star_read_at:new Date().toISOString()}},updated_at:new Date().toISOString()}).eq("id",row.id).eq("updated_at",fresh.data.updated_at).select("id");
